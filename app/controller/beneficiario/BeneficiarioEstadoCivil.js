Ext.define('sisscsj.controller.beneficiario.BeneficiarioEstadoCivil', {
    extend: 'sisscsj.controller.Base',
    stores: [
        'beneficiario.BeneficiarioEstadoCivil'
    ],
    views: [
        'beneficiario.ListaEstadoCivil',
        'beneficiario.Lista'
    ],
    refs: [
        {
    ...
    init: function() {
        this.listen({
            controller: {},
            component: {
                'grid[xtype=beneficiario.listaestadocivil]': {
                    beforerender: this.loadRecords,
                    itemdblclick: this.edit,
                    select: this.habilitarBotones,
                    delselect: this.deshabilitarBotones
                },
                'grid[xtype=beneficiario.listaestadocivil] button#add': {
                    click: this.add
                },
                'grid[xtype=beneficiario.listaestadocivil] button#edit': {
                    click: this.edit2
                },
                'grid[xtype=beneficiario.listaestadocivil] button#delete': {
                    click: this.remove
                },
                'grid[xtype=beneficiario.listaestadocivil] gridview': {
                    itemadd: this.edit
                }
            },
            global: {},
    ...
    habilitarBotones: function ( record, index, eOpts ){
        var me = this;
        var grid = me.getBeneficiarioEstadoCivilLista();
        var botonEdit = grid.down("[xtype='toolbar'] button#edit");
        var botonDelete = grid.down("[xtype='toolbar'] button#delete");
    ...
    deshabilitarBotones: function ( record, index, eOpts ){
        ...
        if (records.length > 0)
        {
            var grid2 = me.getBeneficiarioEstadoCivilLista();
    ...
    add: function(button, e, eOpts) {
        var me = this,
            record = Ext.create('sisscsj.model.beneficiario.BeneficiarioEstadoCivil');
        ...
        else
        {
            ...
            {
                console.log('[BeneficiarioEstadoCivil] Error al recuperar el ID del beneficiario');
                return 'Error';
            }
        }
        // [FIN]
