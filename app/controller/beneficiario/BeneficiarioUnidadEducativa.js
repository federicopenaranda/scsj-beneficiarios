Ext.define('sisscsj.controller.beneficiario.BeneficiarioUnidadEducativa', {
    extend: 'sisscsj.controller.Base',
    stores: [
        'beneficiario.BeneficiarioUnidadEducativa'
    ],
    views: [
        'beneficiario.ListaUnidadEducativa',
        'beneficiario.Lista'
    ],
    refs: [
        {
    ...
    init: function() {
        this.listen({
            controller: {},
            component: {
                'grid[xtype=beneficiario.listaunidadeducativa]': {
                    beforerender: this.loadRecords,
                    itemdblclick: this.edit,
                    select: this.habilitarBotones,
                    delselect: this.deshabilitarBotones
                },
                'grid[xtype=beneficiario.listaunidadeducativa] button#add': {
                    click: this.add
                },
                'grid[xtype=beneficiario.listaunidadeducativa] button#edit': {
                    click: this.edit2
                },
                'grid[xtype=beneficiario.listaunidadeducativa] button#delete': {
                    click: this.remove
                },
                'grid[xtype=beneficiario.listaunidadeducativa] gridview': {
                    itemadd: this.edit
                }
            },
            global: {},
    ...
    habilitarBotones: function ( record, index, eOpts ){
        var me = this;
        var grid = me.getBeneficiarioUnidadEducativaLista();
        var botonEdit = grid.down("[xtype='toolbar'] button#edit");
        var botonDelete = grid.down("[xtype='toolbar'] button#delete");
    ...
    deshabilitarBotones: function ( record, index, eOpts ){
        ...
        if (records.length > 0)
        {
            var grid2 = me.getBeneficiarioUnidadEducativaLista();
    ...
    add: function(button, e, eOpts) {
        var me = this,
            record = Ext.create('sisscsj.model.beneficiario.BeneficiarioUnidadEducativa');
        
        // [INICIO] Se recupera el ID del beneficiario para asociarlo al registro de Estado Civil
        ...
        else
        {
            ...
            {
                console.log('[BeneficiarioUnidadEducativa] Error al recuperar el ID del beneficiario');
                return 'Error';
            }
        }
        // [FIN]
