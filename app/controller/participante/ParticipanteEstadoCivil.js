Ext.define('sisscsj.controller.participante.ParticipanteEstadoCivil', {
    extend: 'sisscsj.controller.Base',
    stores: [
        'participante.ParticipanteEstadoCivil'
    ],
    views: [
        'participante.ListaEstadoCivil',
        'participante.Lista'
    ],
    refs: [
        {
    ...
    init: function() {
        this.listen({
            controller: {},
            component: {
                'grid[xtype=participante.listaestadocivil]': {
                    beforerender: this.loadRecords,
                    itemdblclick: this.edit,
                    select: this.habilitarBotones,
                    delselect: this.deshabilitarBotones
                },
                'grid[xtype=participante.listaestadocivil] button#add': {
                    click: this.add
                },
                'grid[xtype=participante.listaestadocivil] button#edit': {
                    click: this.edit2
                },
                'grid[xtype=participante.listaestadocivil] button#delete': {
                    click: this.remove
                },
                'grid[xtype=participante.listaestadocivil] gridview': {
                    itemadd: this.edit
                }
            },
            global: {},
    ...
    habilitarBotones: function ( record, index, eOpts ){
        var me = this;
        var grid = me.getParticipanteEstadoCivilLista();
        var botonEdit = grid.down("[xtype='toolbar'] button#edit");
        var botonDelete = grid.down("[xtype='toolbar'] button#delete");
    ...
    deshabilitarBotones: function ( record, index, eOpts ){
        ...
        if (records.length > 0)
        {
            var grid2 = me.getParticipanteEstadoCivilLista();
    ...
    edit: function(records, index, node, eOpts) {
        var me = this,
                grid = me.getParticipanteEstadoCivilLista(),
    ...
    add: function(button, e, eOpts) {
        var me = this,
            record = Ext.create('sisscsj.model.participante.ParticipanteEstadoCivil');
        ...
        else
        {
            ...
            {
                console.log('[ParticipanteEstadoCivil] Error al recuperar el ID del participante');
                return 'Error';
            }
        }
        // [FIN]
    ...
    remove: function( button, e, eOpts ) {
        ...
        else
        {
            ...
            else
            {
                Ext.Msg.confirm({
                    title: 'Atención',
                    msg: '¿Esta seguro que desea eliminar este Estado Civil?. Esta acción no puede ser deshecha.',
