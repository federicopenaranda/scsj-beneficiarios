Ext.define('sisscsj.controller.participante.ParticipanteUnidadEducativa', {
    extend: 'sisscsj.controller.Base',
    stores: [
        'participante.ParticipanteUnidadEducativa'
    ],
    views: [
        'participante.ListaUnidadEducativa',
        'participante.Lista'
    ],
    refs: [
        {
    ...
    init: function() {
        this.listen({
            controller: {},
            component: {
                'grid[xtype=participante.listaunidadeducativa]': {
                    beforerender: this.loadRecords,
                    itemdblclick: this.edit,
                    select: this.habilitarBotones,
                    delselect: this.deshabilitarBotones
                },
                'grid[xtype=participante.listaunidadeducativa] button#add': {
                    click: this.add
                },
                'grid[xtype=participante.listaunidadeducativa] button#edit': {
                    click: this.edit2
                },
                'grid[xtype=participante.listaunidadeducativa] button#delete': {
                    click: this.remove
                },
                'grid[xtype=participante.listaunidadeducativa] gridview': {
                    itemadd: this.edit
                }
            },
            global: {},
    ...
    habilitarBotones: function ( record, index, eOpts ){
        var me = this;
        var grid = me.getParticipanteUnidadEducativaLista();
        var botonEdit = grid.down("[xtype='toolbar'] button#edit");
        var botonDelete = grid.down("[xtype='toolbar'] button#delete");
    ...
    deshabilitarBotones: function ( record, index, eOpts ){
        ...
        if (records.length > 0)
        {
            var grid2 = me.getParticipanteUnidadEducativaLista();
    ...
    edit: function(records, index, node, eOpts) {
        var me = this,
                grid = me.getParticipanteUnidadEducativaLista(),
    ...
    add: function(button, e, eOpts) {
        var me = this,
            record = Ext.create('sisscsj.model.participante.ParticipanteUnidadEducativa');
        
        // [INICIO] Se recupera el ID del participante para asociarlo al registro de Estado Civil
        ...
        else
        {
            ...
            {
                console.log('[ParticipanteUnidadEducativa] Error al recuperar el ID del participante');
                return 'Error';
            }
        }
        // [FIN]
    ...
    remove: function( button, e, eOpts ) {
        ...
        // Se esta eliminando un registro del grid de Unidad Educativa
