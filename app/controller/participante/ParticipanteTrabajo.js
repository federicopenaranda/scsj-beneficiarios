Ext.define('sisscsj.controller.participante.ParticipanteTrabajo', {
    extend: 'sisscsj.controller.Base',
    stores: [
        'participante.ParticipanteTrabajo'
    ],
    views: [
        'participante.ListaTrabajo',
        'participante.edit.FormTrabajo',
        'participante.edit.WindowTrabajo'
    ],
    refs: [
        {
        ...
        {
            ref: 'ParticipanteTrabajoWindow',
            selector: '[xtype=participante.edit.windowtrabajo]'
        },
        {
            ref: 'ParticipanteTrabajoForm',
            selector: '[xtype=participante.edit.formtrabajo]'
        },
        {
    ...
    init: function() {
        this.listen({
            controller: {},
            component: {
                'grid[xtype=participante.listatrabajo]': {
                    beforerender: this.loadRecords,
                    itemdblclick: this.edit,
                    select: this.habilitarBotones,
                    delselect: this.deshabilitarBotones
                },
                'grid[xtype=participante.listatrabajo] button#add': {
                    click: this.add
                },
                'grid[xtype=participante.listatrabajo] button#edit': {
                    click: this.edit
                },
                'grid[xtype=participante.listatrabajo] button#delete': {
                    click: this.remove
                },
                'window[xtype=participante.edit.windowtrabajo] button#save': {
                    click: this.save
                },
                'window[xtype=participante.edit.windowtrabajo] button#cancel': {
                    click: this.close
                }
            },
            global: {},
    ...
    loadRecords: function(grid, eOpts) {
        ...
        // Si se esta editando un participante filtrar los trabajos por su ID
    ...
    habilitarBotones: function ( record, index, eOpts ){
        var me = this;
        var grid = me.getParticipanteTrabajoLista();
        var botonEdit = grid.down("[xtype='toolbar'] button#edit");
        var botonDelete = grid.down("[xtype='toolbar'] button#delete");
    ...
    deshabilitarBotones: function ( record, index, eOpts ){
        ...
        if (records.length > 0)
        {
            var grid2 = me.getParticipanteTrabajoLista();
    ...
    add: function(button, e, eOpts) {
        var me = this,
            record = Ext.create('sisscsj.model.participante.ParticipanteTrabajo');
        
        // [INICIO] Se recupera el ID del participante para asociarlo al registro de Trabajo
        ...
        else
        {
            ...
            {
                console.log('[ParticipanteTrabajo] Error al recuperar el ID del participante');
                return 'Error';
            }
        }
        // [FIN]
    ...
    save: function(button, e, eOpts) {
        var me = this,
                grid = me.getParticipanteTrabajoLista(),
                store = grid.getStore(),
    ...
    showEditWindow: function(record) {
        var me = this,
                win = me.getParticipanteTrabajoWindow(),
                isNew = record.phantom;
        // if window exists, show it; otherwise, create new instance
        if (!win) {
            win = Ext.widget('participante.edit.windowtrabajo', {
