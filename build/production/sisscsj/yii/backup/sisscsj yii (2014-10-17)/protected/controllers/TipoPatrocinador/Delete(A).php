    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_tipo_patrocinador'])&& is_numeric($records['id_tipo_patrocinador']) && $records['id_tipo_patrocinador']>0){				
					$model=TipoPatrocinador::model()->deleteByPk($records['id_tipo_patrocinador']);
					if($model==1){
