    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_unidad_educativa'])&& is_numeric($records['id_unidad_educativa']) && $records['id_unidad_educativa']>0){				
					$model=UnidadEducativa::model()->deleteByPk($records['id_unidad_educativa']);
					if($model==1){
