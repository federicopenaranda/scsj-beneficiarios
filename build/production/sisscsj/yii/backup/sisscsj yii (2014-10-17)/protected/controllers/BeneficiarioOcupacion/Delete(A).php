    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_beneficiario_ocupacion'])&& is_numeric($records['id_beneficiario_ocupacion']) && $records['id_beneficiario_ocupacion']>0){				
					$model=BeneficiarioOcupacion::model()->deleteByPk($records['id_beneficiario_ocupacion']);
					if($model==1){
