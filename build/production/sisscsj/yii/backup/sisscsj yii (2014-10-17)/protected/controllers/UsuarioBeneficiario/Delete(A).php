    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_usuario_beneficiario'])&& is_numeric($records['id_usuario_beneficiario']) && $records['id_usuario_beneficiario']>0){				
					$model=UsuarioBeneficiario::model()->deleteByPk($records['id_usuario_beneficiario']);
					if($model==1){
