    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_familia_tipo_casa'])&& is_numeric($records['id_familia_tipo_casa']) && $records['id_familia_tipo_casa']>0){				
					$model=FamiliaTipoCasa::model()->deleteByPk($records['id_familia_tipo_casa']);
					if($model==1){
