	...
	public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['fk_id_privilegios_usuario']) && is_numeric($records['fk_id_privilegios_usuario']) && $records['fk_id_privilegios_usuario']>0){
					$model=PrivilegiosTipoUsuario::model()->deleteAll(array('condition'=>'fk_id_privilegios_usuario=:fk_id_privilegios_usuario and fk_id_tipo_usuario=:fk_id_tipo_usuario','params'=>array(':fk_id_privilegios_usuario'=>$records['fk_id_privilegios_usuario'],':fk_id_tipo_usuario'=>$records['fk_id_tipo_usuario'])));
					if($model==1){
