    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_psicologico'])&& is_numeric($records['id_psicologico']) && $records['id_psicologico']>0){				
					$model=EvalPsicologico::model()->deleteByPk($records['id_psicologico']);
					if($model==1){
