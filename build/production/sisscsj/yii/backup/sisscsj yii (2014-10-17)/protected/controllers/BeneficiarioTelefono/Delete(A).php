    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_beneficiario_telefono'])&& is_numeric($records['id_beneficiario_telefono']) && $records['id_beneficiario_telefono']>0){				
					$model=BeneficiarioTelefono::model()->deleteByPk($records['id_beneficiario_telefono']);
					if($model==1){
