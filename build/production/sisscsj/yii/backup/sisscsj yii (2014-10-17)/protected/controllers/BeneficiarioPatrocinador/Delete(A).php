		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['fk_id_beneficiario']) && is_numeric($records['fk_id_beneficiario']) && $records['fk_id_beneficiario']>0){
					$model=BeneficiarioPatrocinador::model()->deleteAll(array('condition'=>'fk_id_beneficiario=:fk_id_beneficiario and fk_id_patrocinador=:fk_id_patrocinador','params'=>array(':fk_id_beneficiario'=>$records['fk_id_beneficiario'],':fk_id_patrocinador'=>$records['fk_id_patrocinador'])));
					if($model==1){
