    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_privilegios_usuario'])&& is_numeric($records['id_privilegios_usuario']) && $records['id_privilegios_usuario']>0){				
					$model=PrivilegiosUsuario::model()->deleteByPk($records['id_privilegios_usuario']);
					if($model==1){
