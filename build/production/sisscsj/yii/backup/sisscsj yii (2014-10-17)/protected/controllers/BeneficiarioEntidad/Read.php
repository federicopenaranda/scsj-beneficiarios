			...
			if (isset($_GET['limit']) && $_GET['limit']>0 && 
					...
					if (isset($_GET['filter']) && $_GET['filter']!='') {
						...
						if (isset($_GET['sort']) && $_GET['sort']!='') {		
							...
							foreach ($filtro as $parametro) {
								...
								$modelo=$model::model()->with('fkIdBeneficiario','fkIdEntidad')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();			
							}	
						} else {
							foreach ($filtro as $parametro) {
								$condicion=$parametro['property'];
								$valor=$parametro['value'];
								$modelo=$model::model()->with('fkIdBeneficiario','fkIdEntidad')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->findAll();								
							}
						}
						$total="".sizeof($modelo);	
					} else {
						if (isset($_GET['sort']) && $_GET['sort']!='') {
							...
							$condisort=$sort[0]['property'];
							$valorsort=$sort[0]['direction'];	
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdEntidad')->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();		
						} else {
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdEntidad')->pagina($_GET['limit'],$_GET['start'])->findAll();
						}
						$total=$model->count();
					}
				} catch (Exception $e) {
				...
				if ($error=="") {
					...
					foreach($modelo as $staff) {
						$aux=array();
						$aux['id_beneficiario_entidad']					=(int)$staff->id_beneficiario_entidad;
						$aux['fk_id_beneficiario']						=(int)$staff->fk_id_beneficiario;
						$aux['fk_id_entidad']							=(int)$staff->fk_id_entidad;
						$aux['fecha_vinculacion_beneficiario_entidad']	=$staff->fecha_vinculacion_beneficiario_entidad;
						$aux['fecha_retiro_beneficiario_entidad']		=$staff->fecha_retiro_beneficiario_entidad;
						$aux['razon_retiro_beneficiario']				=$staff->razon_retiro_beneficiario;
						$aux['estado_beneficiario_entidad']				=(int)$staff->estado_beneficiario_entidad;
						$aux['fecha_creacion_beneficiario_entidad']		=$staff->fecha_creacion_beneficiario_entidad;
						//***********************************************************
