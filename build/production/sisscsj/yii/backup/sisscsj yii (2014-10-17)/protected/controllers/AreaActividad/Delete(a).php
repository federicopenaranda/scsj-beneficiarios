    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_area_actividad'])&& is_numeric($records['id_area_actividad']) && $records['id_area_actividad']>0){				
					$model=AreaActividad::model()->deleteByPk($records['id_area_actividad']);
					if($model==1){
