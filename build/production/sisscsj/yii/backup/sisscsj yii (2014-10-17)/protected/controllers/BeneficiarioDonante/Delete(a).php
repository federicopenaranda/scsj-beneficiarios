	...
	public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['fk_id_donante']) && is_numeric($records['fk_id_donante']) && $records['fk_id_donante']>0){
					$model=BeneficiarioDonante::model()->deleteAll(array('condition'=>'fk_id_donante=:fk_id_donante and fk_id_beneficiario=:fk_id_beneficiario','params'=>array(':fk_id_donante'=>$records['fk_id_donante'],':fk_id_beneficiario'=>$records['fk_id_beneficiario'])));
					if($model==1){
