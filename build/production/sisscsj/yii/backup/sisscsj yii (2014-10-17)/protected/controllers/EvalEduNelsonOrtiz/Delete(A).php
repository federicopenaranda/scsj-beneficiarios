    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_nelson_ortiz'])&& is_numeric($records['id_nelson_ortiz']) && $records['id_nelson_ortiz']>0){				
					$model=EvalEduNelsonOrtiz::model()->deleteByPk($records['id_nelson_ortiz']);
					if($model==1){
