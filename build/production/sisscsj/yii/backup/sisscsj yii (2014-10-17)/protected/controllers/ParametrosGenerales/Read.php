			...
			if(isset($_GET['limit']) && $_GET['limit']>0 && is_numeric($_GET['limit'])&&isset($_GET['start']) && $_GET['start']>=0 && $_GET['start']<=$_GET['limit'] && isset($_GET['start'])&& is_numeric($_GET['start'])){
				...
				if ($error=="") {
					...
					foreach ($modelo as $staff) {
						$aux=array();
						$aux['id_parametro_general']	=(int)$staff->id_parametro_general;
						$aux['nombre_parametro']		=$staff->nombre_parametro;
						$aux['valor_parametro']			=$staff->valor_parametro;
						$aux['configuracion_parametro']	=$staff->configuracion_parametro;
						$aux['estado_parametro']		=(int)$staff->estado_parametro;
						$arreglo[]=$aux;
					}
