			...
			if (isset($_GET['limit']) && $_GET['limit']>0 && is_numeric($_GET['limit'])&&isset($_GET['start']) && $_GET['start']>=0 && $_GET['start']<=$_GET['limit'] && isset($_GET['start'])&& is_numeric($_GET['start'])){
					...
					if (isset($_GET['filter']) && $_GET['filter']!=''){
						...
						if (isset($_GET['sort']) && $_GET['sort']!=''){		
							...
							foreach ($filtro as $parametro) {
								...
								$modelo=$model::model()->with('fkIdTipoConsulta','fkIdUsuario','fkIdBeneficiario','fkIdVacuna')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();
							}
						}else{
							foreach ($filtro as $parametro) {
								$condicion=$parametro['property'];
								$valor=$parametro['value'];
								$modelo=$model::model()->with('fkIdTipoConsulta','fkIdUsuario','fkIdBeneficiario','fkIdVacuna')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->findAll();
							}
						}
						$total="".sizeof($modelo);
					}else{
						if(isset($_GET['sort']) && $_GET['sort']!=''){
							...
							$condisort=$sort[0]['property'];
							$valorsort=$sort[0]['direction'];	
							$modelo=$model::model()->with('fkIdTipoConsulta','fkIdUsuario','fkIdBeneficiario','fkIdVacuna')->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();		
						}else{
							$modelo=$model::model()->with('fkIdTipoConsulta','fkIdUsuario','fkIdBeneficiario','fkIdVacuna')->pagina($_GET['limit'],$_GET['start'])->findAll();
						}
						$total=$model->count();
					}
				} catch (Exception $e) {
				...
				if ($error=="") {
					...
					foreach($modelo as $staff){
						$aux=array();
						$aux['id_enfermeria']						=(int)$staff->id_enfermeria;
						$aux['fk_id_tipo_consulta']					=(int)$staff->fk_id_tipo_consulta;
						$aux['fk_id_usuario']						=(int)$staff->fk_id_usuario;
						$aux['fk_id_vacuna']						=(int)$staff->fk_id_vacuna;
						$aux['fk_id_beneficiario']					=(int)$staff->fk_id_beneficiario;
						$aux['fecha_enfermeria']					=$staff->fecha_enfermeria;
						$aux['observaciones_enfermeria']			=$staff->observaciones_enfermeria;
						$aux['fecha_creacion_enfermeria']			=$staff->fecha_creacion_enfermeria;
						//***********************************************************
						...
						//**********************************************************
						$aux['id_vacuna']							=(int)$staff->fkIdVacuna->id_vacuna;
						$aux['nombre_vacuna']						=$staff->fkIdVacuna->nombre_vacuna;
						$aux['descripcion_vacuna']					=$staff->fkIdVacuna->descripcion_vacuna;
						$aux['estado_vacuna']						=(int)$staff->fkIdVacuna->estado_vacuna;
						$arreglo[]=$aux;					
					}
