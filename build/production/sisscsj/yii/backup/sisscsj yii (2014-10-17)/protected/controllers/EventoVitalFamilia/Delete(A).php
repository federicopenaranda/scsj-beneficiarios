    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_evento_vital_familia'])&& is_numeric($records['id_evento_vital_familia']) && $records['id_evento_vital_familia']>0){				
					$model=EventoVitalFamilia::model()->deleteByPk($records['id_evento_vital_familia']);
					if($model==1){
