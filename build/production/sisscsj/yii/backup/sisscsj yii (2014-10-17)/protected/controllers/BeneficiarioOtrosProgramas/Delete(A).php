		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['fk_id_beneficiario']) && is_numeric($records['fk_id_beneficiario']) && $records['fk_id_beneficiario']>0){
					$model=BeneficiarioOtrosProgramas::model()->deleteAll(array('condition'=>'fk_id_beneficiario=:fk_id_beneficiario and fk_id_otros_programas=:fk_id_otros_programas','params'=>array(':fk_id_beneficiario'=>$records['fk_id_beneficiario'],':fk_id_otros_programas'=>$records['fk_id_otros_programas'])));
					if($model==1){
