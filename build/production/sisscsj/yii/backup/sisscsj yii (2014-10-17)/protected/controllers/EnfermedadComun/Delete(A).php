    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_enfermedad_comun'])&& is_numeric($records['id_enfermedad_comun']) && $records['id_enfermedad_comun']>0){				
					$model=EnfermedadComun::model()->deleteByPk($records['id_enfermedad_comun']);
					if($model==1){
