    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_area_conocimiento_biblioteca'])&& is_numeric($records['id_area_conocimiento_biblioteca']) && $records['id_area_conocimiento_biblioteca']>0){				
					$model=AreaConocimientoBiblioteca::model()->deleteByPk($records['id_area_conocimiento_biblioteca']);
					if($model==1){
