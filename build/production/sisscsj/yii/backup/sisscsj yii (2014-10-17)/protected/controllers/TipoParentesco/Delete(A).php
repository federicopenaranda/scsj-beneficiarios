    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_tipo_parentesco'])&& is_numeric($records['id_tipo_parentesco']) && $records['id_tipo_parentesco']>0){				
					$model=TipoParentesco::model()->deleteByPk($records['id_tipo_parentesco']);
					if($model==1){
