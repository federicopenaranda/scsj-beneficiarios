    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_tipo_donante'])&& is_numeric($records['id_tipo_donante']) && $records['id_tipo_donante']>0){				
					$model=TipoDonante::model()->deleteByPk($records['id_tipo_donante']);
					if($model==1){
