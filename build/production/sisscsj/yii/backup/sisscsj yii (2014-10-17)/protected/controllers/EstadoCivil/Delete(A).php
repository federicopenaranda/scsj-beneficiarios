    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_estado_civil'])&& is_numeric($records['id_estado_civil']) && $records['id_estado_civil']>0){				
					$model=EstadoCivil::model()->deleteByPk($records['id_estado_civil']);
					if($model==1){
