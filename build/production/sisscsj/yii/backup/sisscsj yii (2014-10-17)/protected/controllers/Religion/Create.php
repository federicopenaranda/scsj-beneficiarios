		...
		if (isset($_GET['callback']) && $_GET['callback']!=='' && !is_numeric($_GET['callback']) && isset($_GET['records'])) {
			...
			try {
				foreach ($params as $value) {
					...
					$model=new Religion();
					try {
						if (isset($records['nombre_religion']) && 
							isset($records['descripcion_religion'])
							) {

							$model->nombre_religion		=$records['nombre_religion'];
							$model->descripcion_religion=$records['descripcion_religion'];
							if ($model->validate()) {
								
								$model->save();
								$id_religion=$model->getPrimaryKey();
		...
		/*if (isset($_GET['records'])){
			...
			if (json_last_error()==JSON_ERROR_NONE) {
				...
				if (isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
					...
					if (isset($records['nombre_religion']) && isset($records['descripcion_religion']) && true){
						$model->nombre_religion=$records['nombre_religion'];
						$model->descripcion_religion=$records['descripcion_religion'];
						//*********************************************************
						$listaRelaciones= array('beneficiario'=>'Beneficiario',
