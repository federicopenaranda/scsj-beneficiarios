    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_beneficiario_tipo'])&& is_numeric($records['id_beneficiario_tipo']) && $records['id_beneficiario_tipo']>0){				
					$model=BeneficiarioTipo::model()->deleteByPk($records['id_beneficiario_tipo']);
					if($model==1){
