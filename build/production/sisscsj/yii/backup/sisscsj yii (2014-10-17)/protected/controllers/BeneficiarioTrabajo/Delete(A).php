    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_beneficiario_trabajo'])&& is_numeric($records['id_beneficiario_trabajo']) && $records['id_beneficiario_trabajo']>0){				
					$model=BeneficiarioTrabajo::model()->deleteByPk($records['id_beneficiario_trabajo']);
					if($model==1){
