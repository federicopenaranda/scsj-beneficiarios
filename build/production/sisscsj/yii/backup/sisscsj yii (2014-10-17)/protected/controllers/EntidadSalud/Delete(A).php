    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_entidad_salud'])&& is_numeric($records['id_entidad_salud']) && $records['id_entidad_salud']>0){				
					$model=EntidadSalud::model()->deleteByPk($records['id_entidad_salud']);
					if($model==1){
