    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_tipo_consulta'])&& is_numeric($records['id_tipo_consulta']) && $records['id_tipo_consulta']>0){				
					$model=TipoConsulta::model()->deleteByPk($records['id_tipo_consulta']);
					if($model==1){
