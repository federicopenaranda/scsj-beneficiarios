			...
			if (isset($_GET['limit']) && $_GET['limit']>0 && is_numeric($_GET['limit'])&&isset($_GET['start']) && $_GET['start']>=0 && $_GET['start']<=$_GET['limit'] && isset($_GET['start'])&& is_numeric($_GET['start'])){
				...
				if ($error=="") {
					...
					foreach ($modelo as $staff) {
						$aux=array();
						$aux['id_vacuna']=(int)$staff->id_vacuna;
						$aux['nombre_vacuna']=$staff->nombre_vacuna;
						$aux['descripcion_vacuna']=$staff->descripcion_vacuna;
						$aux['estado_vacuna']=(int)$staff->estado_vacuna;
						$arreglo[]=$aux;
					}
