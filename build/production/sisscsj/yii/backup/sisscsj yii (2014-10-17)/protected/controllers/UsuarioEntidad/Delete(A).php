    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_usuario_entidad'])&& is_numeric($records['id_usuario_entidad']) && $records['id_usuario_entidad']>0){				
					$model=UsuarioEntidad::model()->deleteByPk($records['id_usuario_entidad']);
					if($model==1){
