    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_tipo_usuario'])&& is_numeric($records['id_tipo_usuario']) && $records['id_tipo_usuario']>0){				
					$model=TipoUsuario::model()->deleteByPk($records['id_tipo_usuario']);
					if($model==1){
