			...
			if(isset($_GET['limit']) && $_GET['limit']>0 && is_numeric($_GET['limit'])&&isset($_GET['start']) && $_GET['start']>=0 && $_GET['start']<=$_GET['limit'] && isset($_GET['start'])&& is_numeric($_GET['start'])){
					...
					if(isset($_GET['filter']) && $_GET['filter']!=''){
						...
						if(isset($_GET['sort']) && $_GET['sort']!=''){		
							...
							foreach ($filtro as $parametro) {
								...
								$modelo=$model::model()->with('fkIdBeneficiario','fkIdEstadoCivil')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();		
							}	
						}else{
							foreach ($filtro as $parametro) {
								$condicion=$parametro['property'];
								$valor=$parametro['value'];
								$modelo=$model::model()->with('fkIdBeneficiario','fkIdEstadoCivil')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->findAll();
							}
						}
						$total="".sizeof($modelo);	
					} else {
						if(isset($_GET['sort']) && $_GET['sort']!=''){
							...
							$condisort=$sort[0]['property'];
							$valorsort=$sort[0]['direction'];	
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdEstadoCivil')->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();		
						}else{
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdEstadoCivil')->pagina($_GET['limit'],$_GET['start'])->findAll();
						}
						$total=$model->count();
					}
				} catch (Exception $e) {
				...
				if ($error=="") {
					...
					foreach($modelo as $staff){
						$aux=array();
						$aux['id_beneficiario_estado_civil']				=(int)$staff->id_beneficiario_estado_civil;
						$aux['fk_id_estado_civil']							=(int)$staff->fk_id_estado_civil;
						$aux['fk_id_beneficiario']							=(int)$staff->fk_id_beneficiario;
						$aux['fecha_asignacion_beneficiario_estado_civil']	=$staff->fecha_asignacion_beneficiario_estado_civil;
						$aux['estado_beneficiario_estado_civil']			=$staff->estado_beneficiario_estado_civil;
						//***********************************************************
						...
						//**********************************************************
						$aux['id_estado_civil']								=(int)$staff->fkIdEstadoCivil->id_estado_civil;
						$aux['nombre_estado_civil']							=$staff->fkIdEstadoCivil->nombre_estado_civil;
						$aux['descripcion_estado_civil']					=$staff->fkIdEstadoCivil->descripcion_estado_civil;
						$arreglo[]=$aux;
					}
