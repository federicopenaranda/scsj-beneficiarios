    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_tipo_actividad'])&& is_numeric($records['id_tipo_actividad']) && $records['id_tipo_actividad']>0){				
					$model=TipoActividad::model()->deleteByPk($records['id_tipo_actividad']);
					if($model==1){
