    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_beneficiario_familia'])&& is_numeric($records['id_beneficiario_familia']) && $records['id_beneficiario_familia']>0){				
					$model=BeneficiarioFamilia::model()->deleteByPk($records['id_beneficiario_familia']);
					if($model==1){
