    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_tipo_problematica'])&& is_numeric($records['id_tipo_problematica']) && $records['id_tipo_problematica']>0){				
					$model=TipoProblematica::model()->deleteByPk($records['id_tipo_problematica']);
					if($model==1){
