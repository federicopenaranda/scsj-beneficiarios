		...
		if (isset($_GET['records'])) {
			...
			if (json_last_error()==JSON_ERROR_NONE) {
				...
				if (isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])) {
					...
					if (isset($records['nombre_tipo_problematica']) && isset($records['descripcion_tipo_problematica']) && true) {
						$model->nombre_tipo_problematica=$records['nombre_tipo_problematica'];
						$model->descripcion_tipo_problematica=$records['descripcion_tipo_problematica'];
						//***************************
						$listaRelaciones= array('eval_psicologico'=>'EvalPsicologico',
											);
						$contNotNull=0;
										...
										foreach ($value as $value2) {
											...
											foreach ($listaRelaciones as $nomtab => $nommod) {
												...
												switch ($key) {
													case $nomtab:
														$obj=new $nommod();
														$obj->fk_id_tipo_problematica=$id;
														try{
