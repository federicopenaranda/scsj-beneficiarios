    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_estado_beneficiario'])&& is_numeric($records['id_estado_beneficiario']) && $records['id_estado_beneficiario']>0){				
					$model=EstadoBeneficiario::model()->deleteByPk($records['id_estado_beneficiario']);
					if($model==1){
