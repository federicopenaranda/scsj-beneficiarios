    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_personal_asistencia'])&& is_numeric($records['id_personal_asistencia']) && $records['id_personal_asistencia']>0){				
					$model=PersonalAsistencia::model()->deleteByPk($records['id_personal_asistencia']);
					if($model==1){
