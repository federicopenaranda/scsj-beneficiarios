...
class ControladorCode extends CCodeModel
{
	...
	public function rules()
	{
		return array_merge(parent::rules(), array(
			array('controller, classRuta, baseClass', 'filter', 'filter'=>'trim'),
			array('controller, baseClass', 'required'),
			array('controller', 'match', 'pattern'=>'/^\w+[\w+\\/]*$/', 'message'=>'{attribute} should only contain word characters and slashes.'),
			array('classRuta', 'match', 'pattern'=>'/^\w+[\w\s,]*$/', 'message'=>'{attribute} should only contain word characters, spaces and commas.'),
			...
			array('baseClass, classRuta', 'sticky'),
		));
	}
	...
	public function prepare()
	{
		...
		$path=Yii::getPathOfAlias('application.views.'.$this->classRuta.'.' .'create') . '.php';
        $code=$this->render($this->templatepath.'/create.php');
        $this->files[]=new CCodeFile($path, $code);
		$path=Yii::getPathOfAlias('application.views.'.$this->classRuta.'.' ."read"). '.php';
		$code=$this->render($this->templatepath.'/read.php');
		$this->files[]=new CCodeFile($path, $code);
		$path=Yii::getPathOfAlias('application.views.'.$this->classRuta.'.' ."update"). '.php';
		$code=$this->render($this->templatepath.'/update.php');
		$this->files[]=new CCodeFile($path, $code);
		$path=Yii::getPathOfAlias('application.views.'.$this->classRuta.'.' ."delete"). '.php';
