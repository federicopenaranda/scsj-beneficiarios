    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_tipo_entidad'])&& is_numeric($records['id_tipo_entidad']) && $records['id_tipo_entidad']>0){				
					$model=TipoEntidad::model()->deleteByPk($records['id_tipo_entidad']);
					if($model==1){
