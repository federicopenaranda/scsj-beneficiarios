	...
	public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback']) && $_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['fk_id_idioma']) && is_numeric($records['fk_id_idioma']) && $records['fk_id_idioma']>0){
					$model=BeneficiarioIdioma::model()->deleteAll(array('condition'=>'fk_id_idioma=:fk_id_idioma and fk_id_beneficiario=:fk_id_beneficiario','params'=>array(':fk_id_idioma'=>$records['fk_id_idioma'],':fk_id_beneficiario'=>$records['fk_id_beneficiario'])));
					if($model==1){
