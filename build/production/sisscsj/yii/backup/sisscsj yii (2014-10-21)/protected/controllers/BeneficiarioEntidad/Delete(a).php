    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_beneficiario_entidad'])&& is_numeric($records['id_beneficiario_entidad']) && $records['id_beneficiario_entidad']>0){				
					$model=BeneficiarioEntidad::model()->deleteByPk($records['id_beneficiario_entidad']);
					if($model==1){
