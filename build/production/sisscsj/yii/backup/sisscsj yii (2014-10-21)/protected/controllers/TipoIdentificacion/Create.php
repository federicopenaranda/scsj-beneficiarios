		...
		if (isset($_GET['records'])){
			...
			if (json_last_error()==JSON_ERROR_NONE) {
				...
				if (isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
					...
					if (isset($records['nombre_tipo_identificacion']) && isset($records['descripcion_tipo_identificacion']) && true){
						$model->nombre_tipo_identificacion=$records['nombre_tipo_identificacion'];
						$model->descripcion_tipo_identificacion=$records['descripcion_tipo_identificacion'];
						//*********************************************************
						$listaRelaciones= array('beneficiario_tipo_identificacion'=>'BeneficiarioTipoIdentificacion',
												);
						$contNotNull=0;
										...
										foreach ($value as $value2) {
											...
											foreach ($listaRelaciones as $nomtab => $nommod) {
												...
												switch ($key) {
													case $nomtab:
														$obj=new $nommod();
														$obj->fk_id_tipo_identificacion=$id;
														try{
