    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_metodo_planificacion_familiar'])&& is_numeric($records['id_metodo_planificacion_familiar']) && $records['id_metodo_planificacion_familiar']>0){				
					$model=MetodoPlanificacionFamiliar::model()->deleteByPk($records['id_metodo_planificacion_familiar']);
					if($model==1){
