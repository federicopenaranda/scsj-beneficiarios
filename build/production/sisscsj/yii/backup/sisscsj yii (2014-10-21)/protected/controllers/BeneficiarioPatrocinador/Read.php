			...
			if(isset($_GET['limit']) && $_GET['limit']>0 && is_numeric($_GET['limit'])&&isset($_GET['start']) && $_GET['start']>=0 && $_GET['start']<=$_GET['limit'] && isset($_GET['start'])&& is_numeric($_GET['start'])){
					...
					if(isset($_GET['filter']) && $_GET['filter']!=''){
						...
						if(isset($_GET['sort']) && $_GET['sort']!=''){		
							...
							foreach ($filtro as $parametro) {
								...
								$modelo=$model::model()->with('fkIdBeneficiario','fkIdPatrocinador')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();			
							}	
						}else{
							foreach ($filtro as $parametro) {
								$condicion=$parametro['property'];
								$valor=$parametro['value'];
								$modelo=$model::model()->with('fkIdBeneficiario','fkIdPatrocinador')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->findAll();
							}
						}
						$total="".sizeof($modelo);	
					}else{
						if(isset($_GET['sort']) && $_GET['sort']!=''){
							...
							$condisort=$sort[0]['property'];
							$valorsort=$sort[0]['direction'];	
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdPatrocinador')->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();		
						}else{
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdPatrocinador')->pagina($_GET['limit'],$_GET['start'])->findAll();
						}
						$total=$model->count();
					}
				} catch (Exception $e) {
				...
				if ($error=="") {
					...
					foreach($modelo as $staff){
						...
						$aux['fk_id_patrocinador']						=(int)$staff->fk_id_patrocinador;
						$aux['numero_caso_beneficiario_patrocinador']	=$staff->numero_caso_beneficiario_patrocinador;
						$aux['numero_ninio_beneficiario_patrocinador']	=$staff->numero_ninio_beneficiario_patrocinador;
						$aux['codigo_donante_beneficiario_patrocinador']=$staff->codigo_donante_beneficiario_patrocinador;
						//***********************************************************
						...
						//**********************************************************
						$aux['id_patrocinador']							=(int)$staff->fkIdPatrocinador->id_patrocinador;
						$aux['fk_id_tipo_patrocinador']					=(int)$staff->fkIdPatrocinador->fk_id_tipo_patrocinador;
						$aux['nombre_patrocinador']						=$staff->fkIdPatrocinador->nombre_patrocinador;
						$aux['apellido_patrocinador']					=$staff->fkIdPatrocinador->apellido_patrocinador;
						$aux['codigo_patrocinador']						=$staff->fkIdPatrocinador->codigo_patrocinador;
						$aux['observaciones_patrocinador']				=$staff->fkIdPatrocinador->observaciones_patrocinador;
						$arreglo[]=$aux;
					}
