	...
	public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['fk_id_unidad_educativa']) && is_numeric($records['fk_id_unidad_educativa']) && $records['fk_id_unidad_educativa']>0){
					$model=BeneficiarioUnidadEducativa::model()->deleteAll(array('condition'=>'fk_id_unidad_educativa=:fk_id_unidad_educativa and fk_id_beneficiario=:fk_id_beneficiario','params'=>array(':fk_id_unidad_educativa'=>$records['fk_id_unidad_educativa'],':fk_id_beneficiario'=>$records['fk_id_beneficiario'])));
					if($model==1){
