		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['fk_id_actividad']) && is_numeric($records['fk_id_actividad']) && $records['fk_id_actividad']>0){
					$model=ActividadAreaActividad::model()->deleteAll(array('condition'=>'fk_id_actividad=:fk_id_actividad and fk_id_sub_area=:fk_id_sub_area','params'=>array(':fk_id_actividad'=>$records['fk_id_actividad'],':fk_id_sub_area'=>$records['fk_id_sub_area'])));
					if($model==1){
