    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_tipo_actor_beneficiario'])&& is_numeric($records['id_tipo_actor_beneficiario']) && $records['id_tipo_actor_beneficiario']>0){				
					$model=TipoActorBeneficiario::model()->deleteByPk($records['id_tipo_actor_beneficiario']);
					if($model==1){
