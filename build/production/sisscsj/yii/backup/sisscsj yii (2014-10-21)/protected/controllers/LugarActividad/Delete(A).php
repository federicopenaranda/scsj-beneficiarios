    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_lugar_actividad'])&& is_numeric($records['id_lugar_actividad']) && $records['id_lugar_actividad']>0){				
					$model=LugarActividad::model()->deleteByPk($records['id_lugar_actividad']);
					if($model==1){
