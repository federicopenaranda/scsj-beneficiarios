    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_departamento'])&& is_numeric($records['id_departamento']) && $records['id_departamento']>0){				
					$model=Departamento::model()->deleteByPk($records['id_departamento']);
					if($model==1){
