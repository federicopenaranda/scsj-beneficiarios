    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_beneficiario_asistencia'])&& is_numeric($records['id_beneficiario_asistencia']) && $records['id_beneficiario_asistencia']>0){				
					$model=BeneficiarioAsistencia::model()->deleteByPk($records['id_beneficiario_asistencia']);
					if($model==1){
