    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_patrocinador'])&& is_numeric($records['id_patrocinador']) && $records['id_patrocinador']>0){				
					$model=Patrocinador::model()->deleteByPk($records['id_patrocinador']);
					if($model==1){
