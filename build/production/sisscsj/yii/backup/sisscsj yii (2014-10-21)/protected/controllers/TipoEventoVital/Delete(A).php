    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_tipo_evento_vital'])&& is_numeric($records['id_tipo_evento_vital']) && $records['id_tipo_evento_vital']>0){				
					$model=TipoEventoVital::model()->deleteByPk($records['id_tipo_evento_vital']);
					if($model==1){
