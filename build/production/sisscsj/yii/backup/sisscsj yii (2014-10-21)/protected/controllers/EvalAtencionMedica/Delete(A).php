    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_atencion_medica'])&& is_numeric($records['id_atencion_medica']) && $records['id_atencion_medica']>0){				
					$model=EvalAtencionMedica::model()->deleteByPk($records['id_atencion_medica']);
					if($model==1){
