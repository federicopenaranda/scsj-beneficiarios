	...
	public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['fk_id_enfermedad_comun']) && is_numeric($records['fk_id_enfermedad_comun']) && $records['fk_id_enfermedad_comun']>0){
					$model=AtencionMedicaEnfermedadComun::model()->deleteAll(array('condition'=>'fk_id_enfermedad_comun=:fk_id_enfermedad_comun and fk_id_atencion_medica=:fk_id_atencion_medica','params'=>array(':fk_id_enfermedad_comun'=>$records['fk_id_enfermedad_comun'],':fk_id_atencion_medica'=>$records['fk_id_atencion_medica'])));
					if($model==1){
