    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_estado_asistencia'])&& is_numeric($records['id_estado_asistencia']) && $records['id_estado_asistencia']>0){				
					$model=EstadoAsistencia::model()->deleteByPk($records['id_estado_asistencia']);
					if($model==1){
