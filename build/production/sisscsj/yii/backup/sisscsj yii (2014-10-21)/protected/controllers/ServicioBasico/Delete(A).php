    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_servicio_basico'])&& is_numeric($records['id_servicio_basico']) && $records['id_servicio_basico']>0){				
					$model=ServicioBasico::model()->deleteByPk($records['id_servicio_basico']);
					if($model==1){
