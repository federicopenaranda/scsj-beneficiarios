    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_tipo_identificacion'])&& is_numeric($records['id_tipo_identificacion']) && $records['id_tipo_identificacion']>0){				
					$model=TipoIdentificacion::model()->deleteByPk($records['id_tipo_identificacion']);
					if($model==1){
