    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_parametro_general'])&& is_numeric($records['id_parametro_general']) && $records['id_parametro_general']>0){				
					$model=ParametrosGenerales::model()->deleteByPk($records['id_parametro_general']);
					if($model==1){
