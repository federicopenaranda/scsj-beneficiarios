    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_odontologia'])&& is_numeric($records['id_odontologia']) && $records['id_odontologia']>0){				
					$model=EvalOdontologia::model()->deleteByPk($records['id_odontologia']);
					if($model==1){
