    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_otros_programas'])&& is_numeric($records['id_otros_programas']) && $records['id_otros_programas']>0){				
					$model=OtrosProgramas::model()->deleteByPk($records['id_otros_programas']);
					if($model==1){
