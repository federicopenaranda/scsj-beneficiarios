    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_gestion_beneficiario'])&& is_numeric($records['id_gestion_beneficiario']) && $records['id_gestion_beneficiario']>0){				
					$model=GestionBeneficiario::model()->deleteByPk($records['id_gestion_beneficiario']);
					if($model==1){
