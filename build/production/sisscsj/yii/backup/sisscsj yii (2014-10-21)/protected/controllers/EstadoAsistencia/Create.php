		...
		if (isset($_GET['records'])){
			...
			if (json_last_error()==JSON_ERROR_NONE) {
				...
				if (isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
					...
					if (isset($records['nombre_estado_asistencia']) && isset($records['descripcion_estado_asistencia']) && true){
						$model->nombre_estado_asistencia=$records['nombre_estado_asistencia'];
						$model->descripcion_estado_asistencia=$records['descripcion_estado_asistencia'];
						
						//***************************
						$listaRelaciones= array('personal_asistencia'=>'PersonalAsistencia',
										...
										foreach ($value as $value2) {
											...
											foreach ($listaRelaciones as $nomtab => $nommod) {
												...
												switch ($key) {
													case $nomtab:
														$obj=new $nommod();
														$obj->fk_id_estado_asistencia=$id;
														try{
