	...
	public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['fk_id_actividad_favorita']) && is_numeric($records['fk_id_actividad_favorita']) && $records['fk_id_actividad_favorita']>0){
					$model=BeneficiarioActividadFavorita::model()->deleteAll(array('condition'=>'fk_id_actividad_favorita=:fk_id_actividad_favorita and fk_id_beneficiario=:fk_id_beneficiario','params'=>array(':fk_id_actividad_favorita'=>$records['fk_id_actividad_favorita'],':fk_id_beneficiario'=>$records['fk_id_beneficiario'])));
					if($model==1){
