    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_estado_entidad'])&& is_numeric($records['id_estado_entidad']) && $records['id_estado_entidad']>0){				
					$model=EstadoEntidad::model()->deleteByPk($records['id_estado_entidad']);
					if($model==1){
