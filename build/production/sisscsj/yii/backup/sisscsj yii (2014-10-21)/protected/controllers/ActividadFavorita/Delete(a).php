	...
	public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback']) && $_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_actividad_favorita']) && is_numeric($records['id_actividad_favorita']) && $records['id_actividad_favorita']>0){				
					$model=ActividadFavorita::model()->deleteByPk($records['id_actividad_favorita']);
					if($model==1){
