			...
			if(isset($_GET['limit']) && $_GET['limit']>0 && is_numeric($_GET['limit'])&&isset($_GET['start']) && $_GET['start']>=0 && $_GET['start']<=$_GET['limit'] && isset($_GET['start'])&& is_numeric($_GET['start'])){
				...
				if ($error=="") {
					...
					foreach($modelo as $staff){
						$aux=array();
						$aux['id_nutricion']						=(int)$staff->id_nutricion;
						...
						$aux['peso_talla_nutricion']				=(float)$staff->peso_talla_nutricion;
						$aux['talla_edad_nutricion']				=(float)$staff->talla_edad_nutricion;
						$aux['fecha_eval_nutricion']				=$staff->fecha_eval_nutricion;
						//***********************************************************
