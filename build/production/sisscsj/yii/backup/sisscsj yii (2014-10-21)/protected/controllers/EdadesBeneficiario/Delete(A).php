    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_edades_beneficiario'])&& is_numeric($records['id_edades_beneficiario']) && $records['id_edades_beneficiario']>0){				
					$model=EdadesBeneficiario::model()->deleteByPk($records['id_edades_beneficiario']);
					if($model==1){
