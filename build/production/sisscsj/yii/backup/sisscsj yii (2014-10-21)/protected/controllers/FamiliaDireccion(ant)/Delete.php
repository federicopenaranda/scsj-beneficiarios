    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_familia_direccion'])&& is_numeric($records['id_familia_direccion']) && $records['id_familia_direccion']>0){				
					$model=FamiliaDireccion::model()->deleteByPk($records['id_familia_direccion']);
					if($model==1){
