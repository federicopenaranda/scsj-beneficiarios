    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if(isset($records['id_eval_computacion'])&& is_numeric($records['id_eval_computacion']) && $records['id_eval_computacion']>0){				
					$model=EvalComputacion::model()->deleteByPk($records['id_eval_computacion']);
					if($model==1){
