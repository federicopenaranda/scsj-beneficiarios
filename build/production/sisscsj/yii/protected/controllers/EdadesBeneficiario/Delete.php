            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_edades_beneficiario'])) {
							$model=EdadesBeneficiario::model()->deleteByPk($record['id_edades_beneficiario']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",EdadesBeneficiario::model()->tableName(),$record['id_edades_beneficiario']);
	                    		$contValRecords++;
	                    	} else {
