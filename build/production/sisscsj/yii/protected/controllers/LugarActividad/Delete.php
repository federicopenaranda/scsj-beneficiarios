            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_lugar_actividad'])) {
							$model=LugarActividad::model()->deleteByPk($record['id_lugar_actividad']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",LugarActividad::model()->tableName(),$record['id_lugar_actividad']);
	                    		$contValRecords++;
	                    	} else {
