            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_enfermeria'])) {
							$model=EvalEnfermeria::model()->deleteByPk($record['id_enfermeria']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",EvalEnfermeria::model()->tableName(),$record['id_enfermeria']);
	                    		$contValRecords++;
	                    	} else {
