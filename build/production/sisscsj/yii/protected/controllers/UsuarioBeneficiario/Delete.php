            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_usuario_beneficiario'])) {
							$model=UsuarioBeneficiario::model()->deleteByPk($record['id_usuario_beneficiario']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",UsuarioBeneficiario::model()->tableName(),$record['id_usuario_beneficiario']);
	                    		$contValRecords++;
	                    	} else {
