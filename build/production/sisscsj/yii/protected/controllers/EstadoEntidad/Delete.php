            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_estado_entidad'])) {
							$model=EstadoEntidad::model()->deleteByPk($record['id_estado_entidad']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",EstadoEntidad::model()->tableName(),$record['id_estado_entidad']);
	                    		$contValRecords++;
	                    	} else {
