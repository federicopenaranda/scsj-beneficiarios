            ...
            try {
				foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
                    	try{
        					$error.= (!isset($record['id_estado_entidad'])) ? "Variable indefinida {id_estado_entidad}" : "";
							...
							if ($error == "") {
								$model=EstadoEntidad::model()->findByPk($record['id_estado_entidad']);							
								$audi=new LogSistema();
								if ($model!==null) {
		                            ...
		                            if ($model->validate()) {
		                                $model->save();
                                        $audi->insertAudi("update",$model->tableName(),$record['id_estado_entidad']);
		                                $contValRecords++;
		                            } else {
