			...
			if (isset($_GET['limit']) && $_GET['limit']>0 && is_numeric($_GET['limit'])&&
				...
				try {
					if (isset($_GET['filter']) && $_GET['filter']!=''){
						...
						if (isset($_GET['sort']) && $_GET['sort']!=''){		
							...
							foreach ($filtro as $parametro) {
								...
								if($condicion=="fk_id_beneficiario")
									$modelo=$model::model()->with('fkIdBeneficiario','fkIdOcupacion')->filterTextoInt($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();
								else
									$modelo=$model::model()->with('fkIdBeneficiario','fkIdOcupacion')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();
							}
						} else {
							foreach ($filtro as $parametro) {
								$condicion=$parametro['property'];
								$valor=$parametro['value'];
								if($condicion=="fk_id_beneficiario")
									$modelo=$model::model()->with('fkIdBeneficiario','fkIdOcupacion')->filterTextoInt($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->findAll();
								else
									$modelo=$model::model()->with('fkIdBeneficiario','fkIdOcupacion')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->findAll();
							}
						}
						$total="".sizeof($modelo);	
					} else {
						...
						if(isset($_GET['sort']) && $_GET['sort']!=''){
							...
							$condisort=$sort[0]['property'];
							$valorsort=$sort[0]['direction'];	
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdOcupacion')->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();		
						}else{
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdOcupacion')->pagina($_GET['limit'],$_GET['start'])->findAll();
						}
						$total=$model->count();
					}
				} catch (Exception $e) {
				...
				if ($error=="") {
					...
					foreach($modelo as $staff){
						$aux=array();
						$aux['id_beneficiario_ocupacion']			=(int)$staff->id_beneficiario_ocupacion;
						$aux['fk_id_beneficiario']					=(int)$staff->fk_id_beneficiario;
						$aux['fk_id_ocupacion']						=(int)$staff->fk_id_ocupacion;
						$aux['fecha_beneficiario_ocupacion']		=$staff->fecha_beneficiario_ocupacion;
						$aux['estado_beneficiario_ocupacion']		=(int)$staff->estado_beneficiario_ocupacion;
						$aux['observacion_beneficiario_ocupacion']	=$staff->observacion_beneficiario_ocupacion;
						//***********************************************************
						...
						//**********************************************************
						#$aux['id_ocupacion']						=(int)$staff->fkIdOcupacion->id_ocupacion;
						$aux['nombre_ocupacion']					=$staff->fkIdOcupacion->nombre_ocupacion;
						#$aux['descripcion_ocupacion']				=$staff->fkIdOcupacion->descripcion_ocupacion;
						$arreglo[]=$aux;
					}
