            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_entidad_marco_logico'])) {
							$model=EntidadMarcoLogico::model()->deleteByPk($record['id_entidad_marco_logico']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",EntidadMarcoLogico::model()->tableName(),$record['id_entidad_marco_logico']);
	                    		$contValRecords++;
	                    	} else {
