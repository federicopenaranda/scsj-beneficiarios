            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_marco_logico'])) {
							$model=MarcoLogico::model()->deleteByPk($record['id_marco_logico']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",MarcoLogico::model()->tableName(),$record['id_marco_logico']);
	                    		$contValRecords++;
	                    	} else {
