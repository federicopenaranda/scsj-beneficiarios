            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_tipo_parentesco'])) {
							$model=TipoParentesco::model()->deleteByPk($record['id_tipo_parentesco']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",TipoParentesco::model()->tableName(),$record['id_tipo_parentesco']);
	                    		$contValRecords++;
	                    	} else {
