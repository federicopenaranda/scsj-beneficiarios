            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_odontologia'])) {
							$model=EvalOdontologia::model()->deleteByPk($record['id_odontologia']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",EvalOdontologia::model()->tableName(),$record['id_odontologia']);
	                    		$contValRecords++;
	                    	} else {
