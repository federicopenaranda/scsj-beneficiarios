            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_beneficiario_estado_civil'])) {
							$model=BeneficiarioEstadoCivil::model()->deleteByPk($record['id_beneficiario_estado_civil']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",BeneficiarioEstadoCivil::model()->tableName(),$record['id_beneficiario_estado_civil']);
	                    		$contValRecords++;
	                    	} else {
