			...
			if (isset($_GET['limit']) && $_GET['limit']>0 && is_numeric($_GET['limit'])&&
						...
						if(isset($_GET['sort']) && $_GET['sort']!=''){		
							...
							foreach ($filtro as $parametro) {
								...
								if($condicion=="fk_id_beneficiario")
									$modelo=$model::model()->with('fkIdBeneficiario')->filterTextoInt($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();
								else	
									$modelo=$model::model()->with('fkIdBeneficiario')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();	
							}
						} else {
							foreach ($filtro as $parametro) {
								$condicion=$parametro['property'];
								$valor=$parametro['value'];
								if($condicion=="fk_id_beneficiario")
									$modelo=$model::model()->with('fkIdBeneficiario')->filterTextoInt($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->findAll();	
								else
									$modelo=$model::model()->with('fkIdBeneficiario')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->findAll();	
							}
						}
						$total="".sizeof($modelo);
					} else {
				...
				if ($error=="") {
					...
					foreach($modelo as $staff){
						$aux=array();
						$aux['id_beneficiario_trabajo']				=(int)$staff->id_beneficiario_trabajo;
						$aux['fk_id_beneficiario']					=(int)$staff->fk_id_beneficiario;
						$aux['monto_ingreso_beneficiario_trabajo']	=(int)$staff->monto_ingreso_beneficiario_trabajo;
						$aux['tipo_trabajo_beneficiario_trabajo']	=$staff->tipo_trabajo_beneficiario_trabajo;
						$aux['estado_beneficiario_trabajo']			=(int)$staff->estado_beneficiario_trabajo;
						$aux['fecha_creacion_beneficiario_trabajo']	=$staff->fecha_creacion_beneficiario_trabajo;
						$aux['descripcion_beneficiario_trabajo']	=$staff->descripcion_beneficiario_trabajo;
						//***********************************************************
