            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_beneficiario_trabajo'])) {
							$model=BeneficiarioTrabajo::model()->deleteByPk($record['id_beneficiario_trabajo']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",BeneficiarioTrabajo::model()->tableName(),$record['id_beneficiario_trabajo']);
	                    		$contValRecords++;
	                    	} else {
