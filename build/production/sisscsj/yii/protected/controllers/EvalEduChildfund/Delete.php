            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_childfund'])) {
							$model=EvalEduChildfund::model()->deleteByPk($record['id_childfund']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",EvalEduChildfund::model()->tableName(),$record['id_childfund']);
	                    		$contValRecords++;
	                    	} else {
