            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_privilegios_usuario'])) {
							$model=PrivilegiosUsuario::model()->deleteByPk($record['id_privilegios_usuario']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",PrivilegiosUsuario::model()->tableName(),$record['id_privilegios_usuario']);
	                    		$contValRecords++;
	                    	} else {
