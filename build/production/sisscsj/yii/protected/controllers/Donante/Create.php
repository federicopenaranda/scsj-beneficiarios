<?php
/**
 * Estas son la accion para el controlador "Donante".
 */
            ...
            try {
				foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
                    	try{
        					$error.= (!isset($record['fk_id_tipo_donante'])) ? "Variable indefinida {fk_id_tipo_donante}" : "";
        					$error.= (!isset($record['nombre_donante'])) ? "Variable indefinida {nombre_donante}" : "";
        					$error.= (!isset($record['descripcion_donante'])) ? "Variable indefinida {descripcion_donante}" : "";
							if ($error=="") {
        						$model->fk_id_tipo_donante=$record['fk_id_tipo_donante'];
        						$model->nombre_donante=$record['nombre_donante'];
        						$model->descripcion_donante=$record['descripcion_donante'];
	                            if ($model->validate()) {
	                                $model->save();
	                                    ...
	                                    if (is_array($valor) && sizeof($valor)!=0) {
	                                    	...
	                                    	if (isset($listaRelaciones[$NomTabRel])) {
		                                        ...
		                                        foreach ($valor as $subvalue) {
                                                    ...
                                                    $audi=new LogSistema();
													$obj->fk_id_donante=$id_tabla_principal;
