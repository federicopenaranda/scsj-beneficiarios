            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_beneficiario_telefono'])) {
							$model=BeneficiarioTelefono::model()->deleteByPk($record['id_beneficiario_telefono']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",BeneficiarioTelefono::model()->tableName(),$record['id_beneficiario_telefono']);
	                    		$contValRecords++;
	                    	} else {
