            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_entidad_estado_entidad'])) {
							$model=EntidadEstadoEntidad::model()->deleteByPk($record['id_entidad_estado_entidad']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",EntidadEstadoEntidad::model()->tableName(),$record['id_entidad_estado_entidad']);
	                    		$contValRecords++;
	                    	} else {
