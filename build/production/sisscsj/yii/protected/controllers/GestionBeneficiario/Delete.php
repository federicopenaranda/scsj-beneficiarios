            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_gestion_beneficiario'])) {
							$model=GestionBeneficiario::model()->deleteByPk($record['id_gestion_beneficiario']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",GestionBeneficiario::model()->tableName(),$record['id_gestion_beneficiario']);
	                    		$contValRecords++;
	                    	} else {
