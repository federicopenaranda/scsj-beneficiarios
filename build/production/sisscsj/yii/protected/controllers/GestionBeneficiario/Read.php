        ...
        if ($error == "") {
			...
			if (isset($_GET['filter']) && $_GET['filter']!='') {
				...
				if (isset($_GET['sort']) && $_GET['sort']!='') {		
					...
					foreach ($filtro as $parametro) {
						...
						if ($condition=="fk_id_beneficiario")
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdGestion')->filterTextoInt($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();			
						else
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdGestion')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();			
					}
					...
					foreach ($filtro as $parametro) {
						...
						$valor=$parametro['value'];
						if ($condition=="fk_id_beneficiario")
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdGestion')->filterTextoInt($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->findAll();
						else
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdGestion')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->findAll();
					}
				}
				$total="".sizeof($modelo);	
			} else {
				...
				if(isset($_GET['sort']) && $_GET['sort']!=''){
					...
					$condisort=$sort[0]['property'];
					$valorsort=$sort[0]['direction'];	
					$modelo=$model::model()->with('fkIdBeneficiario','fkIdGestion')->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();		
				} else {
					$modelo=$model::model()->with('fkIdBeneficiario','fkIdGestion')->pagina($_GET['limit'],$_GET['start'])->findAll();
				}
				$total=$model->count();
			}//filter
			$arreglo=array();
			foreach($modelo as $staff){
				$aux=array();
				$aux['id_gestion_beneficiario']=$staff->id_gestion_beneficiario;
				$aux['fk_id_beneficiario']=$staff->fk_id_beneficiario;
				$aux['fk_id_gestion']=$staff->fk_id_gestion;
				$aux['estado_gestion_beneficiario']=$staff->estado_gestion_beneficiario;
				//***********************************************************
