            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_monitoreo_punto_comunitario'])) {
							$model=MonitoreoPuntoComunitario::model()->deleteByPk($record['id_monitoreo_punto_comunitario']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",MonitoreoPuntoComunitario::model()->tableName(),$record['id_monitoreo_punto_comunitario']);
	                    		$contValRecords++;
	                    	} else {
