            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_metodo_planificacion_familiar'])) {
							$model=MetodoPlanificacionFamiliar::model()->deleteByPk($record['id_metodo_planificacion_familiar']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",MetodoPlanificacionFamiliar::model()->tableName(),$record['id_metodo_planificacion_familiar']);
	                    		$contValRecords++;
	                    	} else {
