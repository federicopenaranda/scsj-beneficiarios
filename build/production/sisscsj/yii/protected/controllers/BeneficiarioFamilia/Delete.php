            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_beneficiario_familia'])) {
							$model=BeneficiarioFamilia::model()->deleteByPk($record['id_beneficiario_familia']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",BeneficiarioFamilia::model()->tableName(),$record['id_beneficiario_familia']);
	                    		$contValRecords++;
	                    	} else {
