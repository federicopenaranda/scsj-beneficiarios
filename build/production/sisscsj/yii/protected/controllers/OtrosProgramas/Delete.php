            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_otros_programas'])) {
							$model=OtrosProgramas::model()->deleteByPk($record['id_otros_programas']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",OtrosProgramas::model()->tableName(),$record['id_otros_programas']);
	                    		$contValRecords++;
	                    	} else {
