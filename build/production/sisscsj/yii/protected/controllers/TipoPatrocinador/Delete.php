            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_tipo_patrocinador'])) {
							$model=TipoPatrocinador::model()->deleteByPk($record['id_tipo_patrocinador']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",TipoPatrocinador::model()->tableName(),$record['id_tipo_patrocinador']);
	                    		$contValRecords++;
	                    	} else {
