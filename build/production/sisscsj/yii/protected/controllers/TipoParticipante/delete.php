            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_tipo_participante'])) {
							$model=TipoParticipante::model()->deleteByPk($record['id_tipo_participante']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",TipoParticipante::model()->tableName(),$record['id_tipo_participante']);
	                    		$contValRecords++;
	                    	} else {
