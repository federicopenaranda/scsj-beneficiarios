            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_tipo_usuario'])) {
							$model=TipoUsuario::model()->deleteByPk($record['id_tipo_usuario']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",TipoUsuario::model()->tableName(),$record['id_tipo_usuario']);
	                    		$contValRecords++;
	                    	} else {
