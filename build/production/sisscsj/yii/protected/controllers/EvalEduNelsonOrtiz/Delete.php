            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_nelson_ortiz'])) {
							$model=EvalEduNelsonOrtiz::model()->deleteByPk($record['id_nelson_ortiz']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",EvalEduNelsonOrtiz::model()->tableName(),$record['id_nelson_ortiz']);
	                    		$contValRecords++;
	                    	} else {
