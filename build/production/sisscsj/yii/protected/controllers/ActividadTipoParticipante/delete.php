            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_actividad_tipo_participante'])) {
							$model=ActividadTipoParticipante::model()->deleteByPk($record['id_actividad_tipo_participante']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ActividadTipoParticipante::model()->tableName(),$record['id_actividad_tipo_participante']);
	                    		$contValRecords++;
	                    	} else {
