        ...
        if ($error == "") {
			...
			if (isset($_GET['filter']) && $_GET['filter']!='') {
				...
				if(isset($_GET['sort']) && $_GET['sort']!=''){		
                    ...
                    $modelo=$model::model()->with('fkIdTipoConsulta','fkIdUsuario','fkIdBeneficiario')->pagina($_GET['limit'],$_GET['start'])->findAll(array("condition"=>$condi,"order"=>$condisort." ".$valorsort));	
				} else {
					foreach ($filtro as $parametro) {
						$condicion=$parametro['property'];
						$valor=$parametro['value'];
                    ...
                    $modelo=$model::model()->with('fkIdTipoConsulta','fkIdUsuario','fkIdBeneficiario')->pagina($_GET['limit'],$_GET['start'])->findAll($condi);
				}
				$total="".sizeof($modelo);
			} else {
			...
			foreach($modelo as $staff){
				$aux=array();
				$aux['id_nutricion']=$staff->id_nutricion;
				$aux['fk_id_tipo_consulta']=$staff->fk_id_tipo_consulta;
				$aux['fk_id_usuario']=$staff->fk_id_usuario;
				$aux['fk_id_beneficiario']=$staff->fk_id_beneficiario;
				$aux['fecha_nutricion']=$staff->fecha_nutricion;
				$aux['observaciones_nutricion']=$staff->observaciones_nutricion;
				$aux['peso_nutricion']=$staff->peso_nutricion;
				$aux['talla_nutricion']=$staff->talla_nutricion;
				$aux['peso_talla_nutricion']=$staff->peso_talla_nutricion;
				$aux['talla_edad_nutricion']=$staff->talla_edad_nutricion;
				//***********************************************************
				$aux['id_tipo_consulta']=$staff->fkIdTipoConsulta->id_tipo_consulta;
