            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_tipo_actividad'])) {
							$model=TipoActividad::model()->deleteByPk($record['id_tipo_actividad']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",TipoActividad::model()->tableName(),$record['id_tipo_actividad']);
	                    		$contValRecords++;
	                    	} else {
