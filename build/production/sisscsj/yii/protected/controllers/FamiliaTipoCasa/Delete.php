            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_familia_tipo_casa'])) {
							$model=FamiliaTipoCasa::model()->deleteByPk($record['id_familia_tipo_casa']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",FamiliaTipoCasa::model()->tableName(),$record['id_familia_tipo_casa']);
	                    		$contValRecords++;
	                    	} else {
