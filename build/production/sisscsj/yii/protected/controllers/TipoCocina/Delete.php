            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_tipo_cocina'])) {
							$model=TipoCocina::model()->deleteByPk($record['id_tipo_cocina']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",TipoCocina::model()->tableName(),$record['id_tipo_cocina']);
	                    		$contValRecords++;
	                    	} else {
