            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_pedagogico'])) {
							$model=EvalPedagogico::model()->deleteByPk($record['id_pedagogico']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",EvalPedagogico::model()->tableName(),$record['id_pedagogico']);
	                    		$contValRecords++;
	                    	} else {
