            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_evento_vital_familia'])) {
							$model=EventoVitalFamilia::model()->deleteByPk($record['id_evento_vital_familia']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",EventoVitalFamilia::model()->tableName(),$record['id_evento_vital_familia']);
	                    		$contValRecords++;
	                    	} else {
