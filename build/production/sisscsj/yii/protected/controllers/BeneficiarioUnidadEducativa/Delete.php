            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_beneficiario_unidad_educativa'])) {
							$model=BeneficiarioUnidadEducativa::model()->deleteByPk($record['id_beneficiario_unidad_educativa']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",BeneficiarioFamilia::model()->tableName(),$record['id_beneficiario_unidad_educativa']);
	                    		$contValRecords++;
	                    	} else {
