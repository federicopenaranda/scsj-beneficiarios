			...
			if (isset($_GET['limit']) && $_GET['limit']>0 && is_numeric($_GET['limit'])&&
					...
					if(isset($_GET['filter']) && $_GET['filter']!=''){
						...
						if(isset($_GET['sort']) && $_GET['sort']!=''){		
							...
							foreach ($filtro as $parametro) {
								...
								if($condicion=="fk_id_beneficiario" || $condicion=="fk_id_unidad_educativa"){
									
									$modelo=$model::model()->with('fkIdBeneficiario','fkIdUnidadEducativa')->filterTextoInt($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();
								}else{
									$modelo=$model::model()->with('fkIdBeneficiario','fkIdUnidadEducativa')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();
							...
							foreach ($filtro as $parametro) {
								...
								$valor=$parametro['value'];
								if($condicion=="fk_id_beneficiario" || $condicion=="fk_id_unidad_educativa"){
									$modelo=$model::model()->with('fkIdBeneficiario','fkIdUnidadEducativa')->filterTextoInt($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->findAll();
									
								} else{		
									$modelo=$model::model()->with('fkIdBeneficiario','fkIdUnidadEducativa')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->findAll();
								}
							}
						}
						$total="".sizeof($modelo);
						...
						if(isset($_GET['sort']) && $_GET['sort']!=''){
							...
							$condisort=$sort[0]['property'];
							$valorsort=$sort[0]['direction'];	
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdUnidadEducativa')->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();		
						}else{
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdUnidadEducativa')->pagina($_GET['limit'],$_GET['start'])->findAll();
						}
						$total=$model->count();
					}
				} catch(Exception $e){
				...
				if ($error=="") {
					...
					foreach($modelo as $staff){
						$aux=array();
						$aux['id_beneficiario_unidad_educativa']			=(int)$staff->id_beneficiario_unidad_educativa;
						$aux['fk_id_unidad_educativa']						=(int)$staff->fk_id_unidad_educativa;
						$aux['fk_id_beneficiario']							=(int)$staff->fk_id_beneficiario;
						$aux['fecha_creacion_beneficiario_unidad_educativa']=$staff->fecha_creacion_beneficiario_unidad_educativa;
						$aux['estado_beneficiario_unidad_educativa']		=(int)$staff->estado_beneficiario_unidad_educativa;
						//***********************************************************
						#$aux['id_unidad_educativa']							=(int)$staff->fkIdUnidadEducativa->id_unidad_educativa;
						$aux['nombre_unidad_educativa']						=$staff->fkIdUnidadEducativa->nombre_unidad_educativa;
						#$aux['telefono_unidad_educativa']					=$staff->fkIdUnidadEducativa->telefono_unidad_educativa;
						#$aux['direccion_unidad_educativa']					=$staff->fkIdUnidadEducativa->direccion_unidad_educativa;
						//**********************************************************
