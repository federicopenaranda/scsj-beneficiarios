            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_estado_asistencia'])) {
							$model=EstadoAsistencia::model()->deleteByPk($record['id_estado_asistencia']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",EstadoAsistencia::model()->tableName(),$record['id_estado_asistencia']);
	                    		$contValRecords++;
	                    	} else {
