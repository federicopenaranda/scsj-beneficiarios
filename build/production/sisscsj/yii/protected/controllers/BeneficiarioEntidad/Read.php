				...
				try {
					if (isset($_GET['filter']) && $_GET['filter']!='') {
						...
						if (isset($_GET['sort']) && $_GET['sort']!='') {		
							...
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdEntidad')->findAll(array("condition"=>$condi,"order"=>$condisort." ".$valorsort,'limit'=>$_GET['limit'],'offset'=>$_GET['start']));
						} else {
							foreach ($filtro as $parametro) {
							...
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdEntidad')->pagina($_GET['limit'],$_GET['start'])->findAll($condi);							
						}
						$total="".sizeof($modelo);	
					} else {
						$modelo = array();
