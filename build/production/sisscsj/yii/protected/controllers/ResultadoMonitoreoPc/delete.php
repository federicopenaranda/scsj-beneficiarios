            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_resultado_monitoreo_pc'])) {
							$model=ResultadoMonitoreoPc::model()->deleteByPk($record['id_resultado_monitoreo_pc']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ResultadoMonitoreoPc::model()->tableName(),$record['id_resultado_monitoreo_pc']);
	                    		$contValRecords++;
	                    	} else {
