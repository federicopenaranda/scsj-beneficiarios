            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_area_conocimiento_biblioteca'])) {
							$model=AreaConocimientoBiblioteca::model()->deleteByPk($record['id_area_conocimiento_biblioteca']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",AreaConocimientoBiblioteca::model()->tableName(),$record['id_area_conocimiento_biblioteca']);
	                    		$contValRecords++;
	                    	} else {
