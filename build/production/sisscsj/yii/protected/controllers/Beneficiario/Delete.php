            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_beneficiario'])) {
							$model=Beneficiario::model()->deleteByPk($record['id_beneficiario']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",Beneficiario::model()->tableName(),$record['id_beneficiario']);
	                    		$contValRecords++;
	                    	} else {
