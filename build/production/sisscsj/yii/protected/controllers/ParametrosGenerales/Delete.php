            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_parametro_general'])) {
							$model=ParametrosGenerales::model()->deleteByPk($record['id_parametro_general']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ParametrosGenerales::model()->tableName(),$record['id_parametro_general']);
	                    		$contValRecords++;
	                    	} else {
