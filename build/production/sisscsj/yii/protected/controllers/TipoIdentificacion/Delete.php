            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_tipo_identificacion'])) {
							$model=TipoIdentificacion::model()->deleteByPk($record['id_tipo_identificacion']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",TipoIdentificacion::model()->tableName(),$record['id_tipo_identificacion']);
	                    		$contValRecords++;
	                    	} else {
