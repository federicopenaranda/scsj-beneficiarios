            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_tipo_monitoreo_actor'])) {
							$model=TipoMonitoreoActor::model()->deleteByPk($record['id_tipo_monitoreo_actor']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",TipoMonitoreoActor::model()->tableName(),$record['id_tipo_monitoreo_actor']);
	                    		$contValRecords++;
	                    	} else {
