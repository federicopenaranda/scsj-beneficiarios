            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_biblioteca'])) {
							$model=Biblioteca::model()->deleteByPk($record['id_biblioteca']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",Biblioteca::model()->tableName(),$record['id_biblioteca']);
	                    		$contValRecords++;
	                    	} else {
