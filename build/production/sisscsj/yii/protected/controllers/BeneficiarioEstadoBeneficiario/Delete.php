            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_beneficiario_estado_beneficiario'])) {
							$model=BeneficiarioEstadoBeneficiario::model()->deleteByPk($record['id_beneficiario_estado_beneficiario']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",BeneficiarioEstadoBeneficiario::model()->tableName(),$record['id_beneficiario_estado_beneficiario']);
	                    		$contValRecords++;
	                    	} else {
