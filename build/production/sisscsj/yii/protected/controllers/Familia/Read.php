			...
			if (isset($_GET['filter']) && $_GET['filter']!='') {
				...
				foreach ($filtro as $parametro) {
					...
					if ($condicion == "primer_nombre_beneficiario" ||
						...
						$condi.= $contFil != sizeof($filtro) ? 'b.'.$condicion." LIKE '%".$valor."%' AND " :  'b.'.$condicion." LIKE '%".$valor."%'";	
						$contFil++;
					} else {
						if ($condicion == 'nombre_donante' ||
							$condicion == "id_donante"
						){
							if ($condicion == "id_donante"){
								$condi.= $contFil != sizeof($filtro) ? 'd.'.$condicion." = ".$valor." AND " :  'd.'.$condicion." = ".$valor;
							} else {
								$condi.= $contFil != sizeof($filtro) ? 'd.'.$condicion." LIKE '%".$valor."%' AND " :  'd.'.$condicion." LIKE '%".$valor."%'";
							}
							$contFil++;
						} else {
...
INNER JOIN beneficiario_donante AS bd ON bd.fk_id_beneficiario = b.id_beneficiario
INNER JOIN donante AS d ON bd.fk_id_donante = d.id_donante';
			if (isset($_GET['filter']) && $_GET['filter']!='')
