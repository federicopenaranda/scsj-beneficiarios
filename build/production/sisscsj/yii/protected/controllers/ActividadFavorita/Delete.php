            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_actividad_favorita'])) {
							$model=ActividadFavorita::model()->deleteByPk($record['id_actividad_favorita']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ActividadFavorita::model()->tableName(),$record['id_actividad_favorita']);
	                    		$contValRecords++;
	                    	} else {
