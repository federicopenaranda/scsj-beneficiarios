            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_beneficiario_tipo'])) {
							$model=BeneficiarioTipo::model()->deleteByPk($record['id_beneficiario_tipo']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",BeneficiarioTipo::model()->tableName(),$record['id_beneficiario_tipo']);
	                    		$contValRecords++;
	                    	} else {
