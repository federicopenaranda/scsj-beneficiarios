            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_beneficiario_patrocinador'])) {
							$model=BeneficiarioPatrocinador::model()->deleteByPk($record['id_beneficiario_patrocinador']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",BeneficiarioPatrocinador::model()->tableName(),$record['id_beneficiario_patrocinador']);
	                    		$contValRecords++;
	                    	} else {
