...
*/
   public function run()
   {
		...
		if ($_GET['id'] && isset($_GET['id'])) {
			...
			$id=CJSON::decode($_GET['id']);
			$tam=sizeof($id['id_beneficiario']);
			$vec=$id['id_beneficiario'];
			...
			for ($i=0; $i <$tam ; $i++) { 
				...
				if (sizeof($res11)!==0) {
					...
					$mPDF1->AddPage();
					$mPDF1->WriteHTML($controller->renderPartial('pdf3',array('res11'=>$res11,'res12'=>$res12,'res14'=>$res14,'res15'=>$res15,'res16'=>$res16,'res17'=>$res17,'res18'=>$res18,'res19'=>$res19,'res110'=>$res110,'res111'=>$res111,'res21'=>$res21,'res22'=>$res22,'res23'=>$res23,'res3'=>$res3,'res31'=>$res31),true));
				}			
			}
				#$mPDF1->Output();
				$mPDF1->Output('REP3.PDF','D');
		} else {
