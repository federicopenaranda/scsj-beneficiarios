...
b.fotografia_beneficiario,
b.numero_caso_beneficiario_patrocinador AS Nro_de_Caso,
c.nombre_curso AS Curso,
n.nombre_nivel AS Nivel,
f.nombre_formacion AS Formacion
FROM
beneficiario AS b
LEFT JOIN religion AS r ON b.fk_id_religion = r.id_religion
LEFT JOIN curso AS c ON b.fk_id_curso = c.id_curso
LEFT JOIN nivel AS n ON b.fk_id_nivel = n.id_nivel
LEFT JOIN formacion as f ON b.fk_id_formacion = f.id_formacion
WHERE
...
bt.monto_ingreso_beneficiario_trabajo AS Ingreso,
c.nombre_curso AS Curso,
fo.nombre_formacion AS Formacion
FROM
beneficiario AS b
INNER JOIN beneficiario_familia AS bf ON bf.fk_id_beneficiario = b.id_beneficiario
...
LEFT JOIN curso AS c ON b.fk_id_curso = c.id_curso
LEFT JOIN formacion as fo ON b.fk_id_formacion = fo.id_formacion
WHERE
f.codigo_familia IN (SELECT
f.codigo_familia
FROM
beneficiario AS b
INNER JOIN beneficiario_familia AS bf ON bf.fk_id_beneficiario = b.id_beneficiario
...
p.fk_id_beneficiario = ".$id;
    ...
    public function historia_social($id){
        $sql="SELECT * 
                FROM  `historia_social` 
                WHERE  `fk_id_beneficiario` =". $id ."
                AND  `estado_historia_social` =1";
        $rows=Yii::app()->db->createCommand($sql)
        ->queryAll();
        return $rows;
    }
