...
tipo_monitoreo_actor.id_tipo_monitoreo_actor = ".$id." ORDER BY nombre_criterio_monitoreo_actor";
	...
	public function fcriterio($id){
		...
		$columns[] = ["text"=>"Participante","dataIndex"=>"fk_id_beneficiario","flex"=>1,"renderer"=>"{{ function( value, metaData, record, rowIndex, colIndex, store, view )  { if (value > 0) { Ext.data.JsonP.request ({ url: sisscsj.app.globals.globalServerPath + \'Beneficiario\',params: {start: \'\',limit: \'\', filter: \'[{\"property\":\"id_beneficiario\",\"value\":\'+ value + \'}]\'},success: function( response, options ) { var tmpUE = response.registros[0]; record.set(\'primer_nombre_beneficiario\', tmpUE.primer_nombre_beneficiario); record.set(\'codigo_beneficiario\', tmpUE.codigo_beneficiario); record.set(\'apellido_paterno_beneficiario\', tmpUE.apellido_paterno_beneficiario); }, failure: function( response, options ) { Ext.Msg.alert( \'Atención\', \'Un error ocurrió durante su petición. Por favor intente nuevamente.\' ); } });  return record.get(\'primer_nombre_beneficiario\') + \' \' + record.get(\'apellido_paterno_beneficiario\') + \' (\' + record.get(\'codigo_beneficiario\') + \')\';   } } }}",/*"renderer"=>"{{function( value, metaData, record, rowIndex, colIndex, store, view )  { console.log(record); return    record.get('primer_nombre_beneficiario' ) + ' ' + record.get('apellido_paterno_beneficiario')} }}",*/"editor"=>['xtype'=>"ux.form.field.remotecombobox","name"=>"fk_id_beneficiario","id"=>"fk_id_beneficiario","displayTpl"=>"{{Ext.create('Ext.XTemplate','<tpl for=\".\">','{primer_nombre_beneficiario} {segundo_nombre_beneficiario} {apellido_paterno_beneficiario} {apellido_materno_beneficiario} ({codigo_beneficiario})','</tpl>') }}","tpl"=>"<tpl for=\'.\'><div class=\'x-boundlist-item\'>{primer_nombre_beneficiario} {segundo_nombre_beneficiario} {apellido_paterno_beneficiario} {apellido_materno_beneficiario} ({codigo_beneficiario})</div></tpl>","valueField"=>"id_beneficiario","store"=>["type"=>"beneficiario.beneficiario"],"editable"=>true,"forceSelection"=>true,"allowBlank"=>false,"typeAhead"=>true,"triggerAction"=>"all","minChars"=>1,"totalProperty"=>"total","pageSize"=>10,"listeners"=>["beforequery"=>"{{ function( queryPlan, eOpts ) { var nQuery =[]; var tmpQuery = { primer_nombre_beneficiario: queryPlan.query,segundo_nombre_beneficiario: queryPlan.query,apellido_paterno_beneficiario: queryPlan.query,apellido_materno_beneficiario: queryPlan.query}; nQuery.push(tmpQuery);queryPlan.query = Ext.encode(nQuery); } }}"]]];
        $sql="SELECT
