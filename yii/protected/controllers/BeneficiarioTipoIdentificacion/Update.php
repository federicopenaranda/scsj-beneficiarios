            ...
            try {
				foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
                    	try{
        					$error.= (!isset($record['id_beneficiario_tipo_identificacion'])) ? "Variable indefinida {id_beneficiario_tipo_identificacion}" : "";
							...
							if ($error == "") {
								$model=BeneficiarioTipoIdentificacion::model()->findByPk($record['id_beneficiario_tipo_identificacion']);
								$audi=new LogSistema();
								if ($model!==null) {
		                            ...
		                            if ($model->validate()) {
		                                $model->save();
										$audi->insertAudi("update",$model->tableName(),$record['id_beneficiario_tipo_identificacion']);
		                                $contValRecords++;
		                            } else {
