			...
			if (isset($_GET['limit']) && $_GET['limit']>0 && is_numeric($_GET['limit'])&&
					...
					if(isset($_GET['filter']) && $_GET['filter']!=''){
						...
						if(isset($_GET['sort']) && $_GET['sort']!=''){		
							...
							foreach ($filtro as $parametro) {
								...
								if($condicion=="fk_id_beneficiario" || $condicion=="fkIdTipoIdentificacion"){
									
									$modelo=$model::model()->with('fkIdBeneficiario','fkIdTipoIdentificacion')->filterTextoInt($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();
								}else{
									$modelo=$model::model()->with('fkIdBeneficiario','fkIdTipoIdentificacion')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();
							...
							foreach ($filtro as $parametro) {
								...
								$valor=$parametro['value'];
								if($condicion=="fk_id_beneficiario" || $condicion=="fkIdTipoIdentificacion"){
									$modelo=$model::model()->with('fkIdBeneficiario','fkIdTipoIdentificacion')->filterTextoInt($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->findAll();
									
								} else{		
									$modelo=$model::model()->with('fkIdBeneficiario','fkIdTipoIdentificacion')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->findAll();
								}
							}
						}
						$total="".sizeof($modelo);
						...
						if(isset($_GET['sort']) && $_GET['sort']!=''){
							...
							$condisort=$sort[0]['property'];
							$valorsort=$sort[0]['direction'];	
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdTipoIdentificacion')->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();		
						}else{
							$modelo=$model::model()->with('fkIdBeneficiario','fkIdTipoIdentificacion')->pagina($_GET['limit'],$_GET['start'])->findAll();
						}
						$total=$model->count();
					}
				} catch(Exception $e){
				...
				if ($error=="") {
					...
					foreach($modelo as $staff){
						$aux=array();
						$aux['id_beneficiario_tipo_identificacion']=(int)$staff->id_beneficiario_tipo_identificacion;
						...
						//***********************************************************
						$aux['id_tipo_identificacion']				=(int)$staff->fkIdTipoIdentificacion->id_tipo_identificacion;
						$aux['nombre_tipo_identificacion']			=$staff->fkIdTipoIdentificacion->nombre_tipo_identificacion;
						$aux['descripcion_tipo_identificacion']		=$staff->fkIdTipoIdentificacion->descripcion_tipo_identificacion;
						$arreglo[]=$aux;
					}	
