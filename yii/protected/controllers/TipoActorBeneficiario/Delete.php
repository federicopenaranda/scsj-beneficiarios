            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_tipo_actor_beneficiario'])) {
							$model=TipoActorBeneficiario::model()->deleteByPk($record['id_tipo_actor_beneficiario']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",TipoActorBeneficiario::model()->tableName(),$record['id_tipo_actor_beneficiario']);
	                    		$contValRecords++;
	                    	} else {
