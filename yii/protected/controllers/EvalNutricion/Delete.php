            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_nutricion'])) {
							$model=EvalNutricion::model()->deleteByPk($record['id_nutricion']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",EvalNutricion::model()->tableName(),$record['id_nutricion']);
	                    		$contValRecords++;
	                    	} else {
