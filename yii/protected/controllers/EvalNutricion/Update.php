    ...
    public function run(){
		...
		if(isset($_GET['records'])){
			...
			if(isset($_GET['callback'])&&$_GET['callback']!=='' && !is_numeric($_GET['callback'])){
				...
				if (isset($records['id_nutricion']) && isset($records['fk_id_tipo_consulta']) && 
					isset($records['fecha_nutricion']) && isset($records['observaciones_nutricion']) && 
					isset($records['peso_talla_nutricion']) && isset($records['talla_edad_nutricion'])
					) {
					...
					if($model!==null){
						if ($model->validaFK('tipo_consulta','id_tipo_consulta',$records['fk_id_tipo_consulta'])!==false &&
        					...
        					$model->peso_nutricion			=$records['peso_nutricion'];
        					$model->talla_nutricion			=$records['talla_nutricion'];
        					$model->peso_talla_nutricion	=$records['peso_talla_nutricion'];
        					$model->talla_edad_nutricion	=$records['talla_edad_nutricion'];

							if($model->save()){
								$audi->insertAudi("update",$model->tableName(),$records['id_nutricion']);
