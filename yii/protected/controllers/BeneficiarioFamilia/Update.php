            ...
            try {
				foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
                    	try{
        					$error.= (!isset($record['id_beneficiario_familia'])) ? "Variable indefinida {id_beneficiario_familia}" : "";
							...
							if ($error == "") {
								$model=BeneficiarioFamilia::model()->findByPk($record['id_beneficiario_familia']);
								$audi=new LogSistema();
								if ($model!==null) {
		                            ...
		                            if ($model->validate()) {
		                                $model->save();
										$audi->insertAudi("update",$model->tableName(),$record['id_beneficiario_familia']);
		                                $contValRecords++;
		                            } else {
