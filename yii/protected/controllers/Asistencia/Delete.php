            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_asistencia'])) {
							$model=Asistencia::model()->deleteByPk($record['id_asistencia']);
							$audi=new LogSistema();	
	                		if ($model==1) {
								$audi->insertAudi("delete",Asistencia::model()->tableName(),$record['id_asistencia']);
	                    		$contValRecords++;
	                    	} else {
