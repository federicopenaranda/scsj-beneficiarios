            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_estado_beneficiario'])) {
							$model=EstadoBeneficiario::model()->deleteByPk($record['id_estado_beneficiario']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",EstadoBeneficiario::model()->tableName(),$record['id_estado_beneficiario']);
	                    		$contValRecords++;
	                    	} else {
