            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_departamento'])) {
							$model=Departamento::model()->deleteByPk($record['id_departamento']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",Departamento::model()->tableName(),$record['id_departamento']);
	                    		$contValRecords++;
	                    	} else {
