            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_usuario_entidad'])) {
							$model=UsuarioEntidad::model()->deleteByPk($record['id_usuario_entidad']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",UsuarioEntidad::model()->tableName(),$record['id_usuario_entidad']);
	                    		$contValRecords++;
	                    	} else {
