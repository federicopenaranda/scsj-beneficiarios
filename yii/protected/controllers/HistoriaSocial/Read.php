			...
			if (isset($_GET['limit']) && $_GET['limit']>0 && is_numeric($_GET['limit'])&&
				...
				try {
					if (isset($_GET['filter']) && $_GET['filter']!=''){
						...
						if(isset($_GET['sort']) && $_GET['sort']!=''){		
							...
							$modelo=$model::model()->with('fkIdBeneficiario')->findAll(array('condition'=>$condi,'order'=>$condisort." ".$valorsort));
							$total="".sizeof($modelo);
							$modelo=$model::model()->with('fkIdBeneficiario')->pagina($_GET['limit'],$_GET['start'])->findAll(array('condition'=>$condi,'order'=>$condisort." ".$valorsort));
				...
				if ($error=="") {
					...
					foreach($modelo as $staff){
						$aux=array();
						$aux['id_historia_social']					=(int)$staff->id_historia_social;
						$aux['fk_id_beneficiario']					=(int)$staff->fk_id_beneficiario;
						$aux['historia_social']						=$staff->historia_social;
						$aux['dinamica_familiar_historia_social']	=$staff->dinamica_familiar_historia_social;
						$aux['situacion_actual_historia_social']	=$staff->situacion_actual_historia_social;
						$aux['estado_historia_social']				=$staff->estado_historia_social;
						//***********************************************************
