            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						...
						if (isset($record['id_historia_social'])) {
							$model=HistoriaSocial::model()->deleteByPk($record['id_historia_social']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",BeneficiarioTrabajo::model()->tableName(),$record['id_historia_social']);
	                    		$contValRecords++;
	                    	} else {
