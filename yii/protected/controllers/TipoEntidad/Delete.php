            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_tipo_entidad'])) {
							$model=TipoEntidad::model()->deleteByPk($record['id_tipo_entidad']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",TipoEntidad::model()->tableName(),$record['id_tipo_entidad']);
	                    		$contValRecords++;
	                    	} else {
