            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_tipo_lugar'])) {
							$model=TipoLugar::model()->deleteByPk($record['id_tipo_lugar']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",TipoLugar::model()->tableName(),$record['id_tipo_lugar']);
	                    		$contValRecords++;
	                    	} else {
