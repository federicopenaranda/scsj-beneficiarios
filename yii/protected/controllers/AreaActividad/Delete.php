            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_area_actividad'])) {
							$model=AreaActividad::model()->deleteByPk($record['id_area_actividad']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",AreaActividad::model()->tableName(),$record['id_area_actividad']);
	                    		$contValRecords++;
	                    	} else {
