            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_resultado_actividad'])) {
							$model=ResultadoActividad::model()->deleteByPk($record['id_resultado_actividad']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ResultadoActividad::model()->tableName(),$record['id_resultado_actividad']);
	                    		$contValRecords++;
	                    	} else {
