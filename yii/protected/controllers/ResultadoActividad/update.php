<?php
/**
 * Estas son la accion para el controlador "ResultadoActividad".
 */
            ...
            try {
				foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
                    	try{
        					$error.= (!isset($record['id_resultado_actividad'])) ? "Variable indefinida {id_resultado_actividad}" : "";
        					$error.= (!isset($record['fk_id_resultado'])) ? "Variable indefinida {fk_id_resultado}" : "";
        					$error.= (!isset($record['fk_id_actividad'])) ? "Variable indefinida {fk_id_actividad}" : "";
							if ($error=="") {
								$model=ResultadoActividad::model()->findByPk($record['id_resultado_actividad']);
								$audi=new LogSistema();
								if ($model!==null) {
        							$model->fk_id_resultado=$record['fk_id_resultado'];
        							$model->fk_id_actividad=$record['fk_id_actividad'];
	                            	if ($model->validate()) {
	                                	$model->save();
	                                	$audi->insertAudi("update",$model->tableName(),$record['id_resultado_actividad']);
	                                    	...
	                                    	if (is_array($valor) && sizeof($valor)!=0) {
	                                    		...
	                                    		if (isset($listaRelaciones[$NomTabRel])) {
		                                        	...
		                                        	foreach ($valor as $subvalue) {
														...
														if ($obj!==null) {
															$obj->fk_id_resultado_actividad=$record['id_resultado_actividad'];
