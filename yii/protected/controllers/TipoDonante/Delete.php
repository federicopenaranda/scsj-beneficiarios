            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_tipo_donante'])) {
							$model=TipoDonante::model()->deleteByPk($record['id_tipo_donante']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",TipoDonante::model()->tableName(),$record['id_tipo_donante']);
	                    		$contValRecords++;
	                    	} else {
