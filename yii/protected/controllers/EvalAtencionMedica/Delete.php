            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_atencion_medica'])) {
							$model=EvalAtencionMedica::model()->deleteByPk($record['id_atencion_medica']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",EvalAtencionMedica::model()->tableName(),$record['id_atencion_medica']);
	                    		$contValRecords++;
	                    	} else {
