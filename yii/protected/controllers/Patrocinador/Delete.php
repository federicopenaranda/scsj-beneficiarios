            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_patrocinador'])) {
							$model=Patrocinador::model()->deleteByPk($record['id_patrocinador']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",Patrocinador::model()->tableName(),$record['id_patrocinador']);
	                    		$contValRecords++;
	                    	} else {
