	...
	public function run()
    {
        ...
        if ($error == "") {
			...
			if(isset($_GET['filter']) && $_GET['filter']!=''){
				...
				if(isset($_GET['sort']) && $_GET['sort']!=''){		
					...
					foreach ($filtro as $parametro) {
						...
						$modelo=$model::model()->with('fkIdMarcoLogico','fkIdEntidad')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();			
					}
				} else {
					foreach ($filtro as $parametro) {
						$condicion=$parametro['property'];
						$valor=$parametro['value'];
						$modelo=$model::model()->with('fkIdMarcoLogico','fkIdEntidad')->filterTexto($condicion,$valor)->pagina($_GET['limit'],$_GET['start'])->findAll();
					}
				}
				$total="".sizeof($modelo);	
			} else {
				if (isset($_GET['sort']) && $_GET['sort']!=''){
					...
					$condisort=$sort[0]['property'];
					$valorsort=$sort[0]['direction'];	
					$modelo=$model::model()->with('fkIdMarcoLogico','fkIdEntidad')->pagina($_GET['limit'],$_GET['start'])->ordenar($condisort,$valorsort)->findAll();		
				} else {
					$modelo=$model::model()->with('fkIdMarcoLogico','fkIdEntidad')->pagina($_GET['limit'],$_GET['start'])->findAll();
				}
				$total=$model->count();
			}
			$arreglo=array();
			foreach($modelo as $staff){
				$aux=array();
				$aux['id_entidad_marco_logico']				=$staff->id_entidad_marco_logico;
				$aux['fk_id_entidad']						=$staff->fk_id_entidad;
				$aux['fk_id_marco_logico']					=$staff->fk_id_marco_logico;
				$aux['estado_entidad_marco_logico']			=$staff->estado_entidad_marco_logico;
					//***********************************************************
