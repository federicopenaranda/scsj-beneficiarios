            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_actividad_proyecto'])) {
							$model=ActividadProyecto::model()->deleteByPk($record['id_actividad_proyecto']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ActividadProyecto::model()->tableName(),$record['id_actividad_proyecto']);
	                    		$contValRecords++;
	                    	} else {
