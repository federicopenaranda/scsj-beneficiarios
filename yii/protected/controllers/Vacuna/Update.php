            ...
            try {
				foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
                    	try{
        					$error.= (!isset($record['id_vacuna'])) ? "Variable indefinida {id_vacuna}" : "";
							...
							if ($error == "") {
								$model=Vacuna::model()->findByPk($record['id_vacuna']);							
								$audi=new LogSistema();
								if ($model!==null) {
		                            ...
		                            if ($model->validate()) {
		                                $model->save();
                                        $audi->insertAudi("update",$model->tableName(),$record['id_vacuna']);
		                                $contValRecords++;
		                            } else {
