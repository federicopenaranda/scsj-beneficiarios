            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						...
						if ($error == "") {
							$model=BeneficiarioTipoIdentificacion::model()->deleteAll(array('condition'=>'fk_id_beneficiario=:fk_id_beneficiario and fk_id_tipo_identificacion=:fk_id_tipo_identificacion','params'=>array(':fk_id_beneficiario'=>$record['fk_id_beneficiario'],':fk_id_tipo_identificacion'=>$record['fk_id_tipo_identificacion'])));
							$audi=new LogSistema();

	                		if ($model==1) {
								$audi->insertAudi("delete",BeneficiarioTipoIdentificacion::model()->tableName(),"");
	                    		$contValRecords++;
	                    	} else {
