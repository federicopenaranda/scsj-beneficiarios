            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_familia_direccion'])) {
							$model=FamiliaDireccion::model()->deleteByPk($record['id_familia_direccion']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",FamiliaDireccion::model()->tableName(),$record['id_familia_direccion']);
	                    		$contValRecords++;
	                    	} else {
