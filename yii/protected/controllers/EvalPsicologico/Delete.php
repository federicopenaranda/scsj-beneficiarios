            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_psicologico'])) {
							$model=EvalPsicologico::model()->deleteByPk($record['id_psicologico']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",EvalPsicologico::model()->tableName(),$record['id_psicologico']);
	                    		$contValRecords++;
	                    	} else {
