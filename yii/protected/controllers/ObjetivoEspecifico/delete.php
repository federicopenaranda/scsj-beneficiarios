            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_objetivo_especifico'])) {
							$model=ObjetivoEspecifico::model()->deleteByPk($record['id_objetivo_especifico']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ObjetivoEspecifico::model()->tableName(),$record['id_objetivo_especifico']);
	                    		$contValRecords++;
	                    	} else {
