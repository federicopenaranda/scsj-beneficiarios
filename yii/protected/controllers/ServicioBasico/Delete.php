            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_servicio_basico'])) {
							$model=ServicioBasico::model()->deleteByPk($record['id_servicio_basico']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ServicioBasico::model()->tableName(),$record['id_servicio_basico']);
	                    		$contValRecords++;
	                    	} else {
