            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_beneficiario_ocupacion'])) {
							$model=BeneficiarioOcupacion::model()->deleteByPk($record['id_beneficiario_ocupacion']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",BeneficiarioOcupacion::model()->tableName(),$record['id_beneficiario_ocupacion']);
	                    		$contValRecords++;
	                    	} else {
