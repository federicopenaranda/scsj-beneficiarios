            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_tipo_problematica'])) {
							$model=TipoProblematica::model()->deleteByPk($record['id_tipo_problematica']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",TipoProblematica::model()->tableName(),$record['id_tipo_problematica']);
	                    		$contValRecords++;
	                    	} else {
