            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_diagnostico'])) {
							$model=Diagnostico::model()->deleteByPk($record['id_diagnostico']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",Diagnostico::model()->tableName(),$record['id_diagnostico']);
	                    		$contValRecords++;
	                    	} else {
