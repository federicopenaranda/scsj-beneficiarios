            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_tipo_evento_vital'])) {
							$model=TipoEventoVital::model()->deleteByPk($record['id_tipo_evento_vital']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",TipoEventoVital::model()->tableName(),$record['id_tipo_evento_vital']);
	                    		$contValRecords++;
	                    	} else {
