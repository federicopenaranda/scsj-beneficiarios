            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						...
						if ($error == "") {
							$model=Parentesco::model()->deleteAll(array('condition'=>'fk_id_beneficiario=:fk_id_beneficiario and fk_id_beneficiario1=:fk_id_beneficiario1','params'=>array(':fk_id_beneficiario'=>$record['fk_id_beneficiario'],':fk_id_beneficiario1'=>$record['fk_id_beneficiario1'])));
							$audi=new LogSistema();

	                		if ($model==1) {
								$audi->insertAudi("delete",tabla::model()->tableName(),"");
	                    		$contValRecords++;
	                    	} else {
