            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_beneficiario_entidad'])) {
							$model=BeneficiarioEntidad::model()->deleteByPk($record['id_beneficiario_entidad']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",BeneficiarioEntidad::model()->tableName(),$record['id_beneficiario_entidad']);
	                    		$contValRecords++;
	                    	} else {
