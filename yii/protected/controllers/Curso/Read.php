        ...
        if ($error == "") {
			...
			try {
				if (isset($_GET['query']) && $_GET['query'] != "") {
					...
					if ($sw == 0) { 
						$arreglo = $model::model()->findAll(array("condition"=>$condiQuery,"offset"=>$_GET['start'],"limit" => $_GET['limit'],"order"=>"nombre_curso"));
					...
					if (isset($_GET['filter']) && $_GET['filter']!='') {
						...
						if (isset($_GET['sort']) && $_GET['sort']!=''){
							...
							$arreglo=$model->pagina($_GET['limit'],$_GET['start'])->findAll(array('order'=>'id_curso'));
						}
						$total=$model->count();
					}
				} //query
