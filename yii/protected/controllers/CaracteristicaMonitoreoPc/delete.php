            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_caracteristica_monitoreo_pc'])) {
							$model=CaracteristicaMonitoreoPc::model()->deleteByPk($record['id_caracteristica_monitoreo_pc']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",CaracteristicaMonitoreoPc::model()->tableName(),$record['id_caracteristica_monitoreo_pc']);
	                    		$contValRecords++;
	                    	} else {
