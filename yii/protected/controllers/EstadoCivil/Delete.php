            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_estado_civil'])) {
							$model=EstadoCivil::model()->deleteByPk($record['id_estado_civil']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",EstadoCivil::model()->tableName(),$record['id_estado_civil']);
	                    		$contValRecords++;
	                    	} else {
