            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						...
						if ($error == "") {
							$model=ActividadTipoActividad::model()->deleteAll(array('condition'=>'fk_id_actividad=:fk_id_actividad and fk_id_tipo_actividad=:fk_id_tipo_actividad','params'=>array(':fk_id_actividad'=>$record['fk_id_actividad'],':fk_id_tipo_actividad'=>$record['fk_id_tipo_actividad'])));
							$audi=new LogSistema();

	                		if ($model==1) {
								$audi->insertAudi("delete",ActividadTipoActividad::model()->tableName(),"");
	                    		$contValRecords++;
	                    	} else {
