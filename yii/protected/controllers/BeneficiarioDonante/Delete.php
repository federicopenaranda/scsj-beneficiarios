            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
        				$error.= (!isset($record['fk_id_donante'])) ? "Variable indefinida {fk_id_donante}" : "";
        				$error.= (!isset($record['fk_id_beneficiario'])) ? "Variable indefinida {fk_id_beneficiario}" : "";
						if ($error == "") {
							$model=BeneficiarioDonante::model()->deleteAll(array('condition'=>'fk_id_donante=:fk_id_donante and fk_id_beneficiario=:fk_id_beneficiario','params'=>array(':fk_id_donante'=>$record['fk_id_donante'],':fk_id_beneficiario'=>$record['fk_id_beneficiario'])));
							$audi=new LogSistema();

	                		if ($model==1) {
								$audi->insertAudi("delete",BeneficiarioDonante::model()->tableName(),"");
	                    		$contValRecords++;
	                    	} else {
