            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_enfermedad_comun'])) {
							$model=EnfermedadComun::model()->deleteByPk($record['id_enfermedad_comun']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",EnfermedadComun::model()->tableName(),$record['id_enfermedad_comun']);
	                    		$contValRecords++;
	                    	} else {
