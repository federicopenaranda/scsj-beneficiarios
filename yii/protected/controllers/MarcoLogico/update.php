            ...
            try {
				foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
                    	try{
        					$error.= (!isset($record['id_marco_logico'])) ? "Variable indefinida {id_marco_logico}" : "";
							...
							if ($error=="") {
								$model=MarcoLogico::model()->findByPk($record['id_marco_logico']);
								$audi=new LogSistema();
								if ($model!==null) {
	                            	...
	                            	if ($model->validate()) {
	                                	$model->save();
	                                	$audi->insertAudi("update",$model->tableName(),$record['id_marco_logico']);
	                                    	...
	                                    	if (is_array($valor) && sizeof($valor)!=0) {
	                                    		...
	                                    		if (isset($listaRelaciones[$NomTabRel])) {
		                                        	...
		                                        	foreach ($valor as $subvalue) {
														...
														if ($obj!==null) {
															$obj->fk_id_marco_logico=$record['id_marco_logico'];
