            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_entidad_salud'])) {
							$model=EntidadSalud::model()->deleteByPk($record['id_entidad_salud']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",EntidadSalud::model()->tableName(),$record['id_entidad_salud']);
	                    		$contValRecords++;
	                    	} else {
