            ...
            try {
				foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
                    	try{
        					$error.= (!isset($record['id_gestion_beneficiario'])) ? "Variable indefinida {id_gestion_beneficiario}" : "";
							...
							if ($error=="") {
								$model=GestionBeneficiario::model()->findByPk($record['id_gestion_beneficiario']);
                               	$audi=new LogSistema();
								if ($model!==null) {
	                            	...
	                            	if ($model->validate()) {
	                                	$model->save();
	                                	$audi->insertAudi("update",$model->tableName(),$record['id_gestion_beneficiario']);
		                                        	...
		                                        	foreach ($valor as $subvalue) {
														...
														if (sizeof($listaNomPri)==1) {
															...
															if (sizeof($listaNomPri)==2 && $t==0) {
																$obj->deleteAll("fk_id_gestion_beneficiario=?",array($record['id_gestion_beneficiario']));
																$t=1-$t;
															}
														}
														if ($obj!==null) {
															$obj->fk_id_gestion_beneficiario=$record['id_gestion_beneficiario'];	
