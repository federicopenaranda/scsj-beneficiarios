            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_escolaridad'])) {
							$model=Escolaridad::model()->deleteByPk($record['id_escolaridad']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",Escolaridad::model()->tableName(),$record['id_escolaridad']);
	                    		$contValRecords++;
	                    	} else {
