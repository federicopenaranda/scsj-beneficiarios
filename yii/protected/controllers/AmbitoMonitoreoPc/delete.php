            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_ambito_monitoreo_pc'])) {
							$model=AmbitoMonitoreoPc::model()->deleteByPk($record['id_ambito_monitoreo_pc']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",AmbitoMonitoreoPc::model()->tableName(),$record['id_ambito_monitoreo_pc']);
	                    		$contValRecords++;
	                    	} else {
