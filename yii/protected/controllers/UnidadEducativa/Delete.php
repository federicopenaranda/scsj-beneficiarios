            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_unidad_educativa'])) {
							$model=UnidadEducativa::model()->deleteByPk($record['id_unidad_educativa']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",UnidadEducativa::model()->tableName(),$record['id_unidad_educativa']);
	                    		$contValRecords++;
	                    	} else {
