        ...
        if ($error == "") {
			...
			try {
				if (isset($_GET['query']) && $_GET['query'] != "") {
					...
					if ($sw == 0) { 
						$arreglo = $model::model()->findAll(array("condition"=>$condiQuery,"offset"=>$_GET['start'],"limit" => $_GET['limit']));
						$total = sizeof($arreglo);
					} else {
						$arreglo = $model::model()->findAll(array("condition"=>$condiQuery));
						$total = sizeof($arreglo);
						$arreglo = $model::model()->findAll(array("condition"=>$condiQuery,"offset"=>$_GET['start'],"limit" => $_GET['limit'],"order"=>"nombre_unidad_educativa"));
					}
				} else {
