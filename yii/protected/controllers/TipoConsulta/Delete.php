            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_tipo_consulta'])) {
							$model=TipoConsulta::model()->deleteByPk($record['id_tipo_consulta']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",TipoConsulta::model()->tableName(),$record['id_tipo_consulta']);
	                    		$contValRecords++;
	                    	} else {
