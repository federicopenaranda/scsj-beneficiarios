            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_resultado_evaluacion'])) {
							$model=ResultadoEvaluacion::model()->deleteByPk($record['id_resultado_evaluacion']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ResultadoEvaluacion::model()->tableName(),$record['id_resultado_evaluacion']);
	                    		$contValRecords++;
	                    	} else {
