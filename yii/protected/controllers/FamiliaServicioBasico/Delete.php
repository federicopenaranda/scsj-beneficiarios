            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_familia_servicio_basico'])) {
							$model=FamiliaServicioBasico::model()->deleteByPk($record['id_familia_servicio_basico']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",FamiliaServicioBasico::model()->tableName(),$record['id_familia_servicio_basico']);
	                    			$contValRecords++;
	                    	} else {
