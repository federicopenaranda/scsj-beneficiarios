            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_objetivo_general'])) {
							$model=ObjetivoGeneral::model()->deleteByPk($record['id_objetivo_general']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ObjetivoGeneral::model()->tableName(),$record['id_objetivo_general']);
	                    		$contValRecords++;
	                    	} else {
