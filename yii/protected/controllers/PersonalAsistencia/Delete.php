            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_personal_asistencia'])) {
							$model=PersonalAsistencia::model()->deleteByPk($record['id_personal_asistencia']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",PersonalAsistencia::model()->tableName(),$record['id_personal_asistencia']);
	                    		$contValRecords++;
	                    	} else {
