            ...
            try {
				foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
                    	try{
        					$error.= (!isset($record['id_monitoreo_punto_comunitario'])) ? "Variable indefinida {id_monitoreo_punto_comunitario}" : "";
							...
							if ($error=="") {
								$model=MonitoreoPuntoComunitario::model()->findByPk($record['id_monitoreo_punto_comunitario']);
								$audi=new LogSistema();
								if ($model!==null) {
	                            	...
	                            	if ($model->validate()) {
	                                	$model->save();
	                                	$audi->insertAudi("update",$model->tableName(),$record['id_monitoreo_punto_comunitario']);
	                                    	...
	                                    	if (is_array($valor) && sizeof($valor)!=0) {
	                                    		...
	                                    		if (isset($listaRelaciones[$NomTabRel])) {
		                                        	...
		                                        	foreach ($valor as $subvalue) {
														...
														if ($obj!==null) {
															$obj->fk_id_monitoreo_punto_comunitario=$record['id_monitoreo_punto_comunitario'];
