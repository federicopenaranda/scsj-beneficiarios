            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_criterio_monitoreo_actor'])) {
							$model=CriterioMonitoreoActor::model()->deleteByPk($record['id_criterio_monitoreo_actor']);
                            $audi=new LogSistema();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",CriterioMonitoreoActor::model()->tableName(),$record['id_criterio_monitoreo_actor']);
	                    		$contValRecords++;
	                    	} else {
