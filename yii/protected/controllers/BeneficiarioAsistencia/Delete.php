            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_beneficiario_asistencia'])) {
							$model=BeneficiarioAsistencia::model()->deleteByPk($record['id_beneficiario_asistencia']);
							$audi=new LogSistema();
	                		if ($model==1) {
								$audi->insertAudi("delete",BeneficiarioAsistencia::model()->tableName(),$record['id_beneficiario_asistencia']);
	                    		$contValRecords++;
	                    	} else {
